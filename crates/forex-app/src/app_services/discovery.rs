...
use forex_data::{
    ensure_timeframes_with_resample, load_symbol_dataset, prepare_multitimeframe_features, FeatureCache,
    MANDATORY_TFS,
};
use forex_search::{
...
pub fn start_discovery_job(
    ...
    log_discovery_event("ui_discovery_job", "STARTED", format!("starting discovery for {}", request.symbol));

    tokio::spawn(async move {
        if cancel.is_requested() {
            let cancelled = cancelled_snapshot_from(snapshot, "operator cancelled discovery before data load");
            send_event(&tx, ServiceEvent::DiscoveryUpdated(cancelled.clone()));
            log_discovery_event("ui_discovery_job", "CANCELLED", cancelled.report.summary.clone());
            return;
        }

        let load_request = request.clone();
        let dataset = match tokio::task::spawn_blocking(move || load_symbol_dataset(&load_request.data_root, &load_request.symbol)).await {
            ...
            Err(err) => {
                let failed = failed_snapshot_from(snapshot, anyhow::anyhow!("discovery data load join error: {err}"));
                send_event(&tx, ServiceEvent::DiscoveryUpdated(failed.clone()));
                log_discovery_event("ui_discovery_job", "FAILED", failed.report.summary.clone());
                return;
            }
        ...
        if cancel.is_requested() {
            let cancelled = cancelled_snapshot_from(snapshot, "operator cancelled discovery after data load");
            send_event(&tx, ServiceEvent::DiscoveryUpdated(cancelled.clone()));
            log_discovery_event("ui_discovery_job", "CANCELLED", cancelled.report.summary.clone());
            return;
        }
        ...
        let feature_build = tokio::task::spawn_blocking(move || {
            let dataset = ensure_timeframes_with_resample(&dataset, &feature_request.base_tf, MANDATORY_TFS)?;
            let higher_refs: Vec<&str> = feature_request.higher_tfs.iter().map(|tf| tf.as_str()).collect();
            ...
            let base_ohlcv = dataset
                ...
                .cloned()
                .ok_or_else(|| anyhow::anyhow!("base timeframe missing: {}", feature_request.base_tf))?;
        ...
        let (features, base_ohlcv) = match feature_build {
            ...
            Err(err) => {
                let failed = failed_snapshot_from(snapshot, anyhow::anyhow!("feature preparation join error: {err}"));
                send_event(&tx, ServiceEvent::DiscoveryUpdated(failed.clone()));
                log_discovery_event("ui_discovery_job", "FAILED", failed.report.summary.clone());
                return;
            }
        ...
        if cancel.is_requested() {
            let cancelled = cancelled_snapshot_from(snapshot, "operator cancelled discovery before portfolio construction");
            send_event(&tx, ServiceEvent::DiscoveryUpdated(cancelled.clone()));
            log_discovery_event("ui_discovery_job", "CANCELLED", cancelled.report.summary.clone());
            return;
        }
        ...
        let search_result = tokio::task::spawn_blocking(move || {
            ...
            ensure_non_empty_portfolio(&result, &format!("{} {}", search_request.symbol, search_request.base_tf))?;

            let out_path = PathBuf::from("cache")
                .join("discovery")
                .join(format!("{}_{}.json", search_request.symbol, search_request.base_tf));
        ...
        let result = match search_result {
            ...
            Err(err) => {
                let failed = failed_snapshot_from(snapshot, anyhow::anyhow!("discovery join error: {err}"));
                send_event(&tx, ServiceEvent::DiscoveryUpdated(failed.clone()));
                log_discovery_event("ui_discovery_job", "FAILED", failed.report.summary.clone());
                return;
            }
        ...
        log_discovery_event("ui_discovery_job", "SUCCESS", completed.report.summary.clone());
    });
...
mod tests {
    use super::*;
    use crate::app_services::jobs::{JobKind, JobSnapshot, JobState};
    ...
    fn invalid_request_fails_before_launch() {
        ...
        let err = request.validate().expect_err("expected invalid request to fail");
