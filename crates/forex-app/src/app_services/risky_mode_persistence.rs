...
//! 2. `<dirs::config_dir>/forex-ai/risky_mode_state.json` — `%APPDATA%`
//!    on Windows, `$XDG_CONFIG_HOME` on Linux, `~/Library/Application
//!    Support` on macOS.
//! 3. `<cwd>/.local/forex-ai/risky_mode_state.json` — dev machine
//!    fallback.
//!
//! # Why a sibling file (not extending `Settings` or `WizardStateFile`)
//!
//! - `Settings` lives in `forex-core` and is reloaded by every
//!   component. Adding risky-mode fields there would touch every
//!   `Settings::default()` call site + add forex-core schema churn.
...
//!   established forex-app convention for "one persisted contract per
//!   sibling file".
//!
//! # Schema versioning
//!
...
pub struct RiskyModeStateFile {
    ...
    /// `forex_core::RiskyModeConfig::default().starting_capital_usd`
...
/// 2. `<dirs::config_dir>/forex-ai/risky_mode_state.json`
/// 3. `<cwd>/.local/forex-ai/risky_mode_state.json`
///
...
pub fn save_risky_mode_state(state: &RiskyModeStateFile) -> Result<()> {
    ...
    tracing::info!(
        target: "forex_app::risky_mode_persistence",
        path = %path.display(),
...
pub fn load_risky_mode_state() -> Result<Option<RiskyModeStateFile>> {
    ...
    if let Err(err) = forex_core::check_schema_version_readable(&state, "risky_mode_state.json") {
        tracing::error!(
            target: "forex_app::risky_mode_persistence",
            path = %path.display(),
            error = %err,
...
mod tests {
    ...
    fn unique_temp_state_path(label: &str) -> PathBuf {
        ...
        let ts = std::time::SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        std::env::temp_dir().join(format!("forex-ai-rms-{label}-{pid}-{ts}.json"))
    }
