...
#[cfg(test)]
pub fn completed_snapshot(completed_models: Vec<String>, failed_models: Vec<String>) -> JobSnapshot {
    completed_snapshot_from(JobSnapshot::new(JobKind::Training), completed_models, failed_models)
}
...
pub fn start_training_job(
    ...
    log_training_event("ui_training_job", "STARTED", format!("starting training for {}", request.symbol));

    tokio::spawn(async move {
        if cancel.is_requested() {
            let cancelled = cancelled_snapshot_from(snapshot, "operator cancelled training before settings load");
            send_event(&tx, ServiceEvent::TrainingUpdated(cancelled.clone()));
            log_training_event("ui_training_job", "CANCELLED", cancelled.report.summary.clone());
            return;
        }
        ...
        .await
        {
            ...
            Err(err) => {
                let failed = failed_snapshot_from(snapshot, anyhow::anyhow!("training settings join error: {err}"));
                send_event(&tx, ServiceEvent::TrainingUpdated(failed.clone()));
                log_training_event("ui_training_job", "FAILED", failed.report.summary.clone());
                return;
            }
        ...
        if cancel.is_requested() {
            let cancelled = cancelled_snapshot_from(snapshot, "operator cancelled training before backend execution");
            send_event(&tx, ServiceEvent::TrainingUpdated(cancelled.clone()));
            log_training_event("ui_training_job", "CANCELLED", cancelled.report.summary.clone());
            return;
        }
        ...
        match train_result {
            Ok(Ok(())) => {
                ...
                log_training_event("ui_training_job", "SUCCESS", completed.report.summary.clone());
            }
            Ok(Err(err)) => {
                let failed = failed_snapshot_from(snapshot, err);
                send_event(&tx, ServiceEvent::TrainingUpdated(failed.clone()));
            ...
            Err(err) => {
                let failed = failed_snapshot_from(snapshot, anyhow::anyhow!("training join error: {err}"));
                send_event(&tx, ServiceEvent::TrainingUpdated(failed.clone()));
                log_training_event("ui_training_job", "FAILED", failed.report.summary.clone());
            }
        }
