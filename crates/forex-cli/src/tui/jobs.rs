...
//! spawn a child `forex-cli` subprocess and stream its stderr lines into
...
impl JobManager {
    ...
    pub fn spawn_with_env(
        ...
        // run an older `~/bin/forex-cli.exe` and confuse everyone).
        let exe = std::env::current_exe().unwrap_or_else(|_| std::path::PathBuf::from("forex-cli"));
