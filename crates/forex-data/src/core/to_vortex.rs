...
//! becomes "what the rest of forex-data may consume". Downstream code
...
//!   one of `forex_core::CANONICAL_TIMEFRAMES`. H2 is explicitly
...
pub struct IngestionSchema {
    ...
    /// If provided, it MUST be in `forex_core::CANONICAL_TIMEFRAMES`.
        ...
        if median >= lo && median <= hi {
            bail!(
                "validate: row spacing implies H2 timeframe (median delta = {median} ms). \
                 H2 is not in forex_core::CANONICAL_TIMEFRAMES (cTrader does not expose H2)."
            );
        }
    }
...
mod tests {
    ...
    ///   crates/forex-data/tests/fixtures/EURUSD_M5_real.csv
    /// (operator rule: NO synthetic data ever). Until that fixture
    /// exists the test stays `#[ignore]`d.
    #[test]
    #[ignore = "needs real-data fixture: crates/forex-data/tests/fixtures/EURUSD_M5_real.csv"]
    fn csv_to_vortex_round_trip_real_data() {
        ...
        let tmp = std::env::temp_dir().join("forex_ai_to_vortex_round_trip.vortex");
