use forex_models::tree_models::{CatBoostExpert, LightGBMExpert, XGBoostExpert};
...
fn compiled_tree_feature_set_is_not_empty() {
    ...
    assert!(
        any_tree_backend,
        "forex-models should expose at least one compiled tree backend in the default tree stack"
    );
}
