...
pub use forex_core::contracts::DeterminismPolicy;
...
/// Convenience entry point that installs both the backtest and quality
/// runtime overrides from the legacy `FOREX_BOT_*` env vars in a single
/// call. Production binaries (`forex-cli`, `forex-app`) should invoke this
/// once at startup so that `forex-search` itself never reads the
/// environment again during a run.
