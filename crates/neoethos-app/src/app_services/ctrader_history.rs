...
/// [`neoethos_core::CANONICAL_TIMEFRAMES`] and not natively supported by
...
pub fn fetch_historical_bars_with_transport<T: CTraderOpenApiTransport>(
    ...
    if !neoethos_core::is_canonical_timeframe(&request.timeframe) {
        return Err(anyhow!(
...
mod tests {
    ...
    /// `crates/neoethos-app/tests/fixtures/ctrader/order_list_by_position_id/README.md`.
    /// Wired into:
    ...
    fn rejects_h2_historical_bars_request() {
        ...
        // because H2 is not in `neoethos_core::CANONICAL_TIMEFRAMES` and
