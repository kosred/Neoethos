...
impl ProductionCTraderLiveAuthBackend {
        ...
        loop {
            match listener.accept() {
                Ok((stream, peer)) => {
                    accept_count += 1;
                    tracing::info!(
                        target: "neoethos_app::ctrader::oauth",
                        step = "accept",
                        accept_count,
                        peer = %peer,
    ...
    fn read_authorization_code_from_stream(
        ...
        // desktop app. We also surface a "return to NeoEthos" message
...
<meta charset=\"utf-8\">\
<title>NeoEthos - cTrader login received</title>\
...
<p>Return to <strong>NeoEthos</strong> to continue.</p>\
<p style=\"color:#5a6068\">This tab will close automatically.</p>\
<div class=\"brand\">NeoEthos &middot; Pure Rust Terminal</div>\
</div>\
        ...
        let (callback_port, listener) = self.bind_loopback_listener(&request.loopback)
            .with_context(|| format!(
                "OAuth step 1/5 (bind_loopback) failed — could not bind any of the allowed callback ports {:?} on host {}. \
                 Another process is likely holding the port; close any other NeoEthos instance or pick a different port range in settings.",
