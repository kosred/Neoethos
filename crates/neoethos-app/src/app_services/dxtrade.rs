...
fn dxtrade_user_agent() -> String {
    format!("neoethos/{}", env!("CARGO_PKG_VERSION"))
}
...
/// entropy. Format: `neoethos-<32-hex>`. The DXtrade spec
...
fn generate_order_code() -> String {
    ...
    if rand::rngs::OsRng.try_fill_bytes(&mut bytes).is_err() {
        ...
        return format!("neoethos-fallback-{hi:016x}{lo:016x}");
    }
...
mod tests {
    ...
    fn submit_market_order_posts_to_canonical_orders_url_with_auth() {
        ...
        assert!(outcome.broker_order_id.starts_with("neoethos-"));
    ...
    fn submit_market_order_emits_single_order_request_body() {
        ...
        assert!(order_code.starts_with("neoethos-"));
    }
    ...
    fn modify_order_puts_to_orders_url_with_existing_order_code() {
        ...
        let outcome = backend
            .modify_order(&dummy_session(), "neoethos-deadbeef", &order)
            .expect("modify ok");

        assert_eq!(outcome.broker_order_id, "neoethos-deadbeef");
        ...
        assert_eq!(body["orderCode"], "neoethos-deadbeef");
    ...
    fn cancel_order_deletes_canonical_url_with_auth() {
        ...
        backend
            .cancel_order(&dummy_session(), "neoethos-deadbeef")
            .expect("cancel ok");
        ...
        assert_eq!(
            calls[0].url,
            "https://demo.dx.trade/dxsca-web/accounts/acc-1/orders/neoethos-deadbeef"
        );
        assert_eq!(calls[0].bearer.as_deref(), Some("token"));
    ...
    fn cancel_surfaces_409_conflict_when_order_is_final() {
        ...
        let backend = ProductionDxTradeOrderBackend::with_transport(transport);
        let err = backend
            .cancel_order(&dummy_session(), "neoethos-x")
            .expect_err("must bail");
        let msg = err.to_string();
    ...
    #[test]
    fn generate_order_code_carries_neoethos_prefix() {
        let code = generate_order_code();
        assert!(code.starts_with("neoethos-"), "code = {code}");
    }
