...
//! ([`neoethos_models::EnsemblePredictor`]) and the live-bar
...
//!     ├─ convert &[HistoricalBar] → neoethos_data::Ohlcv
//!     ├─ neoethos_data::compute_hpc_feature_frame  → FeatureFrame
...
//!
//! `neoethos_models::base::ExpertModel::predict_proba` documents
//! `Array2<f32>` shape `(N, 3)` with **`[neutral, buy, sell]`**
//! column order. The aggregator [`neoethos_models::SoftVotingEnsemble`]
...
use neoethos_data::{FeatureFrame, FeatureProfile, Ohlcv, compute_hpc_feature_frame};
use neoethos_models::EnsemblePredictor;
...
/// into a [`neoethos_data::Ohlcv`] suitable for the feature builder.
...
/// `neoethos_models::parallel_trainer::TrainingPayload::from_named_dense`
...
mod tests {
    use super::*;
    use neoethos_models::SoftVotingEnsemble;
    use neoethos_models::ensemble_inference::{
        ExpertLoadOutcome, ExpertModel, ExpertOutputKind, ExpertPrediction,
    };
    use neoethos_models::runtime::capabilities::ModelFamily;
