...
//!   returns `neoethos_core::CANONICAL_TIMEFRAMES` only. cTrader's API also
...
pub(super) fn chart_history_window_ms(timeframe: &str) -> Option<i64> {
    ...
    // from neoethos-data's resample logic.
    let minutes = neoethos_data::parse_timeframe_to_minutes(timeframe).ok()?;
...
pub(super) fn run_ctrader_bootstrap_batch_with_context(
    ...
    snapshot.report.log_path = Some("logs/neoethos.log".to_string());
