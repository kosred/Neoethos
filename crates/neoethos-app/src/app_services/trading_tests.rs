...
// neoethos_core::symbol_metadata to resolve pip values without hitting a
...
// covered exhaustively by `neoethos_core::domain::risky_mode::tests`.
...
fn signed_risky_mode_config() -> neoethos_core::RiskyModeConfig {
    let mut cfg = neoethos_core::RiskyModeConfig::default();
...
fn risky_mode_gate_rejects_order_when_kill_switch_tripped() {
    ...
    assert_eq!(result, Err(neoethos_core::KillSwitchTier::PerDay));
}
...
fn halt_button_also_trips_risky_mode_kill_switch() {
    ...
    assert_eq!(tier, neoethos_core::KillSwitchTier::Manual);
    ...
    assert_eq!(result, Err(neoethos_core::KillSwitchTier::Manual));
}
