...
impl ForexApp {
    ...
    fn refresh_symbols(&mut self) {
        self.state.available_symbols =
            match neoethos_data::discover_symbols(&self.state.runtime.data_dir) {
                Ok(s) => s,
                Err(err) => {
                    tracing::warn!(
                        target: "neoethos_app::main",
        ...
        while let Ok(msg) = self.rx.try_recv() {
            match msg {
                ...
                ServiceEvent::Heartbeat => {
                    ...
                    if dispatched > 0 {
                        tracing::debug!(
                            target: "neoethos_app::auto_trade",
                            dispatched,
...
fn compact_status_text(status: &str) -> String {
    ...
    {
        tracing::info!(
            target: "neoethos_app::status_msg",
