...
// `POST /broker/account/select` DELETED 2026-08-10 (audit #120).
//
// It set the ACTIVE cTrader account — the single most consequential broker
// setting there is — and it had no client anywhere: the desktop app selects an
// account through the Tauri command `select_account`
// (`desktop/src-tauri/src/broker.rs:256`, called from `desktop/src/api.ts:194`).
// The two did not agree. The Tauri command also sets `enabled_for_execution` on
// the chosen account and switches the Demo/Live environment to match its type;
// this route only reordered the list, so after it ran `resolve_creds` could still
// prefer a DIFFERENT account (it takes `enabled_for_execution` first and only
// falls back to `.first()`, `broker_api.rs:276-282`).
//
// `docs/codex-control-plane.md` requires the HTTP control plane to be
// structurally incapable of flipping the system Demo→Live or changing which
// account trades. Deleting the route is what makes that structural. Account
// selection stays where the operator already does it, in the desktop app.
//
// `AccountSelectDto`, `SelectOutcome` and `account_select` went with it, together
// with the route registration in `server/mod.rs`.
