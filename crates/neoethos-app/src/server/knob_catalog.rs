...
fn build_catalog() -> Vec<KnobEntry> {
    ...
    vec![
        ...
        KnobEntry {
            ...
            help_short: "Flat generations before the combo HARD early-stops and the run advances to the next pair/timeframe. 0 = never (run to the time cap).",
            help_long: "After the soft diversity kick fails to break stagnation for this many generations, the search stops wasting the wall-clock budget and moves on — coverage (more pairs) beats depth on a converged combo. 0 disables (legacy behaviour: grind until prop_search_max_hours). Heavy timeframes that plateau early benefit most.",
