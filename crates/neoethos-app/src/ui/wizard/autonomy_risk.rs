...
/// analysis §5.1 cites eToro 40 % but defaults neoethos to a tighter
...
pub fn render(ui: &mut egui::Ui, controller: &mut WizardController) -> StepResult {
    ...
    // to `<config_dir>/neoethos/risky_mode_state.json`. At next app
        ...
        .show(ui, |ui| {
            ui.label(
                egui::RichText::new(format!(
                    ...
                    neoethos_core::MAX_ACCEPTABLE_INITIAL_RUIN_PROBABILITY * 100.0
                ))
                .strong()
    ...
    if ack_now != ack_before {
        controller.config.risky_mode_ruin_ceiling_acknowledged = if ack_now {
            Some(neoethos_core::MAX_ACCEPTABLE_INITIAL_RUIN_PROBABILITY)
        } else {
            None
        };
    ...
    ui.horizontal(|ui| {
        ...
        ui.add_enabled_ui(quiz_passed, |ui| {
                ...
                .clicked()
            {
                ...
                tracing::info!(
                    target: "neoethos_app::wizard::risk_quiz",
...
///   1. ASCII domain-separator `"neoethos-risk-quiz-v"` (prevents
///      cross-protocol collisions if some other neoethos feature
...
pub fn compute_quiz_answer_hash(quiz_version: u32, answers: &[&str]) -> String {
    ...
    hasher.update(b"neoethos-risk-quiz-v");
