...
//!
//! Drives `crates/neoethos-app/src/app_services/ctrader_history.rs::
...
//! the current build; the `neoethos-cli` mirror will call them when its
...
use crate::ui::theme;
use neoethos_data::{Ohlcv, write_symbol_timeframe_vortex};
...
/// `neoethos_data::symbol_timeframe_vortex_path` so the wizard's writes
/// match the loader's reads byte-for-byte.
pub fn canonical_vortex_path(root: &Path, symbol: &str, timeframe: &str) -> PathBuf {
    neoethos_data::symbol_timeframe_vortex_path(root, symbol, timeframe)
}
...
pub fn build_job_list(symbols: &[String], timeframes: &[String]) -> Vec<HistoryJob> {
    ...
    for symbol in symbols {
        for timeframe in timeframes {
            ...
            // `neoethos_core::CANONICAL_TIMEFRAMES`, but if a non-
            // canonical entry leaked into the config we reject here
            // too (H2 must never reach the wizard layer).
            if !neoethos_core::is_canonical_timeframe(timeframe) {
                tracing::warn!(
                    target: "neoethos_app::wizard::historical",
                    symbol = %symbol,
                    timeframe = %timeframe,
...
mod tests {
    ...
    fn partial_and_complete_sentinel_paths_are_siblings() {
        let path = PathBuf::from("/tmp/neoethos/symbol=EURUSD/timeframe=M1/data.vortex");
    ...
    fn build_job_list_filters_non_canonical_timeframes() {
        ...
        // H2 is intentionally not in `neoethos_core::CANONICAL_TIMEFRAMES`
    ...
    fn wizard_tempdir() -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!("neoethos-wizard-history-{pid}-{nanos}"));
