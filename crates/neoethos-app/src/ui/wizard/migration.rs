...
//! neoethos pre-0.5 was portable (all state under `~/.neoethos/`); the
...
use neoethos_core::storage::json::temporary_path;
...
pub fn portable_candidate_roots() -> Vec<PathBuf> {
    ...
    if let Some(home) = dirs::home_dir() {
        roots.push(home.join(".neoethos"));
        roots.push(home.join("neoethos"));
    }
    // Windows-style `%USERPROFILE%/.neoethos` is covered by `home_dir`
    // on Windows.
    roots
}
...
/// `neoethos_core::storage::json::write_json_atomic` so the operator's
    ...
    if let Some(parent) = dest.parent() {
        if let Ok(d) = fs::File::open(parent) {
            ...
            if let Err(err) = d.sync_all() {
                tracing::debug!(
                    target: "neoethos_app::ui::wizard::migration",
                    dir = %parent.display(),
                    error = %err,
...
mod tests {
    ...
    fn tmp_dir(label: &str) -> PathBuf {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        p.push(format!("neoethos-wizard-migration-{}-{}", label, nanos));
    ...
    /// `migration_skip_on_no_portable_install` — empty `~/.neoethos/`,
    ...
    fn migration_returns_empty_outcome_when_source_root_missing() {
        ...
        let bogus = std::env::temp_dir().join("neoethos-this-does-not-exist-and-never-will");
    ...
    /// Ship-gate §5.1.6 — "Migration of legacy `~/.neoethos/` data is
