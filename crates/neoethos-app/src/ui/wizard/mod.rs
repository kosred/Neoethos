...
//! - The wizard lives entirely inside `neoethos-app::ui::wizard`. No new
//!   crate is introduced.
//! - The state machine (`WizardController`) is renderer-agnostic. The
//!   egui surface here and the ratatui surface in
//!   `neoethos-cli/src/tui/wizard.rs` both drive the same controller.
...
//!   Step 10. The IO uses
//!   `neoethos_core::storage::json::write_json_atomic` for crash
//!   safety (audit-cleaned at F-CORE2-018).
//!
//! Operator invariants enforced here (NOT in step files):
//! - 11 canonical timeframes from `neoethos_core::contracts::temporal`
//!   (NO H2). See `symbols.rs`.
//! - 4 % monthly profit *floor* from
//!   `neoethos_core::domain::prop_firm::PropFirmConstraints::FTMO_STANDARD`.
...
pub struct WizardConfig {
    ...
    // binary at build time (see crates/neoethos-app/build.rs::
    // emit_embedded_credentials + crates/neoethos-app/src/app_services/
    ...
    /// re-prompts. f64 to match the rebuilt neoethos_core::RiskyModeConfig
...
/// `neoethos-cli wizard` (ratatui) own one of these per session.
///
...
/// egui entry point. Called from `neoethos-app::main` when the wizard
/// gate fires (first-run sentinel missing OR `--wizard` CLI flag).
///
/// Renders the current step. Renderers are wrapped in
/// `egui::Window::new("neoethos Setup Wizard")` to match the modal
