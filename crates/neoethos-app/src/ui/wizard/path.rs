...
//! - macOS:   `~/Library/Application Support/neoethos/`
//! - Linux:   `$XDG_DATA_HOME/neoethos/` (i.e. `~/.local/share/neoethos/`)
//!
//! On entry the wizard also runs `migration::detect_portable_install`
//! to surface the legacy `~/.neoethos/` layout (spec §6).
//!
...
//! consts). Kept public for the `neoethos-cli` mirror and the spec.
#![allow(dead_code)]
...
pub const WIZARD_DEFAULT_DATA_FOLDER: &str = "neoethos";
...
fn pick_folder_via_rfd(seed: &std::path::Path) -> Option<PathBuf> {
    ...
    rfd::FileDialog::new()
        .set_title("Choose neoethos data folder")
        .set_directory(seed)
        .pick_folder()
}
...
pub fn render(ui: &mut egui::Ui, controller: &mut WizardController) -> StepResult {
    ...
    ui.label(
        egui::RichText::new("Choose where neoethos will store data, models, and OAuth tokens.")
    ...
    if let Some(report) = portable {
        ui.separator();
        ui.label(
            egui::RichText::new("Legacy ~/.neoethos detected")
                .strong()
                .color(theme::WARNING),
        );
...
mod tests {
    ...
    #[test]
    fn default_data_path_ends_in_neoethos_when_resolvable() {
    ...
    fn rfd_picker_returns_override_in_tests() {
        ...
        // wiring under test.
        let probe = std::env::temp_dir().join("neoethos-rfd-test-path");
