...
//! discipline (`neoethos_core::storage::json::write_json_atomic`,
//! audit-cleaned at F-CORE2-018). Both the egui (`mod.rs`) and the
//! ratatui (`neoethos-cli`) front-ends drive the data type.
//!
//! References:
...
use anyhow::Result;
use neoethos_core::storage::json::{read_json, write_json_atomic};
...
/// concurrent neoethos-app instance detect a stale file.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
...
impl WizardStateFile {
    ...
    /// state files detectable from a concurrent `neoethos-app` instance.
    ///
...
mod tests {
    ...
    fn unique_temp_dir(label: &str) -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("neoethos-wizard-state-{label}-{pid}-{nanos}"));
    ...
    fn default_path_joins_filename_under_config_dir() {
        let p = WizardStateFile::default_path(Path::new("/etc/neoethos"));
        assert_eq!(p, Path::new("/etc/neoethos").join(WIZARD_STATE_FILENAME));
    }
}
