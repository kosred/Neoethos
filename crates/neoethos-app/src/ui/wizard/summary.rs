...
//!    timestamp; `sha2 = "0.10"` is now a direct dep of `neoethos-app`
...
//! `neoethos_core::storage::json::write_json_atomic` (audit-cleaned at
...
//! the current build; the consts are public so the `neoethos-cli` mirror
...
use eframe::egui;
use neoethos_core::CANONICAL_TIMEFRAMES;
use neoethos_core::PropFirmConstraints;
use neoethos_core::Settings;
use neoethos_core::storage::json::write_json_atomic;
use neoethos_core::system::{HardwareProbe, HardwareProfile};
...
/// `WizardConfig` into a `neoethos_core::Settings` so the on-disk
...
/// Project the wizard's snapshot config onto `neoethos_core::Settings`.
///
...
pub fn wizard_config_to_settings(data_path: &Path, controller: &WizardController) -> Settings {
    ...
    tracing::info!(
        target: "neoethos_app::ui::wizard::summary",
...
/// time (see `crates/neoethos-app/build.rs::emit_embedded_credentials`).
...
pub fn write_broker_credentials(controller: &WizardController) -> Result<()> {
    ...
    if let Some(bundle) = crate::ui::wizard::oauth::expose_token_bundle() {
        ...
        tracing::info!(
            target: "neoethos_app::wizard::apply",
...
/// `<config_dir>/neoethos/risky_mode_state.json` (sibling file, not
...
mod tests {
    ...
    fn unique_temp_dir(label: &str) -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("neoethos-wizard-apply-{label}-{pid}-{nanos}"));
    ...
    fn wizard_state_path_returns_default_when_data_path_set() {
        ...
        c.config.data_path = Some(PathBuf::from("/tmp/neoethos-wsp"));
        let p = wizard_state_path(&c).expect("path");
        assert_eq!(p, PathBuf::from("/tmp/neoethos-wsp/wizard_state.json"));
    }
