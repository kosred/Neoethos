...
//! is a policy constant referenced from the spec and from the `neoethos-cli`
...
/// this file: `crates/neoethos-app/src/ui/wizard/welcome.rs` →
/// `<repo_root>/LICENSE`.
...
fn platform_license_candidates() -> Vec<PathBuf> {
    ...
    // macOS `Contents/MacOS/neoethos-app` → `../Resources/LICENSE`, and
    ...
    if cfg!(target_os = "linux") {
        out.push(PathBuf::from("/usr/share/neoethos/LICENSE"));
        out.push(PathBuf::from("/usr/share/doc/neoethos/LICENSE"));
    } else if cfg!(target_os = "macos") {
        out.push(PathBuf::from(
            "/Applications/NeoEthos.app/Contents/Resources/LICENSE",
        ));
    }
...
mod tests {
    ...
    fn load_license_text_reads_from_env_var() {
        ...
        tmp.push(format!("neoethos-license-test-{}.txt", std::process::id()));
    ...
    fn load_license_text_falls_back_to_compiled_in_when_disk_missing() {
        ...
        let mut tmp = std::env::temp_dir();
        tmp.push(format!(
            "neoethos-license-does-not-exist-{}.txt",
            std::process::id()
        ));
