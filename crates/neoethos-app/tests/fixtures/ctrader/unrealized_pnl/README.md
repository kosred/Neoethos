...
`crates/neoethos-app/src/app_services/pnl.rs::tests`. Synthetic broker
...
`crates/neoethos-app/src/app_services/ctrader_money.rs::scale_ctrader_money_int`
...
2. Wrap the `ProductionCTraderOpenApiTransport::send_sequence` call
   with a one-line `tracing::debug!(target: "neoethos_app::pnl_capture",
...
3. Run a real reconcile + PnL fetch, redirecting the trace output to a
   file. `RUST_LOG=neoethos_app::pnl_capture=debug cargo run ...` is
   enough.
