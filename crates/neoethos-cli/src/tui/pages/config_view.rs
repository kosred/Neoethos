//! Config page — renders [`neoethos_core::resolved_config::ResolvedConfig`]
...
pub fn draw(area: Rect, buf: &mut Buffer, _shared: &AppShared) {
    ...
    let settings = neoethos_core::Settings::load().unwrap_or_default();
    ...
    for f in &resolved.display_fields {
        let source_color = match f.source {
            neoethos_core::resolved_config::ResolvedSource::Config => theme::TEXT_PRIMARY,
            neoethos_core::resolved_config::ResolvedSource::SentinelExpanded => theme::ACCENT,
            neoethos_core::resolved_config::ResolvedSource::EnvOverride => theme::SELL,
            neoethos_core::resolved_config::ResolvedSource::Default => theme::TEXT_MUTED,
        };
