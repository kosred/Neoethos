...
//! `neoethos-app/src/ui/wizard/` defines the canonical state machine
//! (`WizardController`) and per-step renderer signatures; this file
//! is the placeholder so that `neoethos-cli wizard` is wired into
//! `main.rs` as a recognised subcommand. The actual ratatui draw
//! routines (§8.1 mapping table) require a `neoethos-cli` ↔ `neoethos-app`
//! dep on the shared `WizardController` data type, which today
//! lives behind `neoethos_app::ui::wizard` — that requires either
...
fn run_wizard_tui_with_terminal_state(stdin_is_tty: bool) -> Result<()> {
    if !stdin_is_tty {
        ...
        return Err(anyhow::anyhow!("neoethos-cli wizard: stdin is not a tty"));
    }

    // FIXME(wizard-tui): port the per-step ratatui pages from the
    // desktop wizard (see `neoethos-app/src/ui/wizard/`):
