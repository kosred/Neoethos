...
//!   [`neoethos_core::CANONICAL_TIMEFRAMES`] (e.g. `EURUSD_H2.csv` lands in
...
impl DatasetDiscovery {
    ...
    pub fn scan(root: impl AsRef<Path>) -> Result<Self> {
            ...
            .into_iter()
        {
            ...
            // canonical list lives in neoethos-core and is the single
            // source of truth.
            if let Some(tf_label) = timeframe_raw.as_ref()
                && !neoethos_core::is_canonical_timeframe(tf_label)
            {
                skipped.push(SkippedFile {
                    path,
