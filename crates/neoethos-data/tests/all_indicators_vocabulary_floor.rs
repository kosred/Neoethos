//! The test that would have caught the 341-indicator silent collapse.
//!
//! # What went wrong, and why nothing noticed for sixteen months
//!
//! `hpc_ta` iterated all 342 ids in [`ALL_INDICATORS`] and kept whatever came
//! back through an `if let Ok(output)` with no `else` and an `if v.len() == n`
//! with no `else`. Exactly ONE of the 342 (`ttm_trend`) produced a column. The
//! other 341 were discarded with no log line, no counter, and no way for any
...
//! (three are moving-average family dispatch selectors, five have no dispatch
...
use neoethos_data::core::indicator_ledger::{
    EXPECTED_NON_PRODUCING, EXPECTED_NON_PRODUCING_OUTPUTS, expected_non_producing,
    expected_non_producing_output,
};
...
fn all_indicators_produce_columns_or_the_build_fails() {
    ...
    let attemptable = declared - excluded;
    ...
    // original defect in miniature: the 341-drop was an OUTPUT-level failure
    // (92 of the 342 failed on `output_id: None`, and every multi-output
    ...
    let floor_columns: usize = ALL_INDICATORS
        ...
        .map(|id| {
            vector_ta::indicators::registry::get_indicator(id)
                .map_or(1, |info| info.outputs.len().max(1))
        })
        .sum();

    let silent: Vec<&&str> = ALL_INDICATORS
        .iter()
        .filter(|id| !per_id.contains_key(**id) && expected_non_producing(id).is_none())
        .collect();
    ...
    for &id in ALL_INDICATORS {
        ...
        if info.outputs.len() <= 1 {
            ...
            for o in &info.outputs {
                // An output vector-ta declares but cannot serve is named, with
                // the file:line of the contradiction, in
                // EXPECTED_NON_PRODUCING_OUTPUTS. Same contract as the id-level
                // list: still attempted, still counted, and a success makes the
                // entry stale.
                if expected_non_producing_output(id, o.id).is_some() {
                    continue;
                }
    ...
    // The output exclusion list can rot exactly like the id one, so it gets the
    // same staleness gate.
    let stale_outputs: Vec<String> = EXPECTED_NON_PRODUCING_OUTPUTS
        .iter()
        .filter(|(id, o, _)| emitted.contains(format!("{id}_{o}").as_str()))
        .map(|(id, o, _)| format!("{id}_{o}"))
        .collect();
    ...
    eprintln!("total columns         : {}   (floor {floor_columns})", cols.len());
    ...
    if !missing_outputs.is_empty() {
        let head: Vec<&str> = missing_outputs.iter().take(20).map(|s| s.as_str()).collect();
        eprintln!("  {}", head.join(", "));
    }
    ...
    assert!(
        ...
        "{} output(s) are named in EXPECTED_NON_PRODUCING_OUTPUTS but DID produce a column: {:?}\n\
         Delete the entry — it is now lowering the floor for no reason.",
        stale_outputs.len(),
        stale_outputs
    );
    assert!(
    ...
    // is precisely how 341 of them vanished for sixteen months. The band stays
    ...
    assert!(
        ...
        "the historical dispatch produced {historical} columns on these bars, which is ABOVE this \
         test's floor of {floor_ids} — so this test would NOT have caught the 341-indicator \
