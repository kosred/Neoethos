...
#[test]
fn classic_evwma_v8_separates_fixed_n_from_rolling_and_binds_exact_cuda_sources() {
    let evwma = source(
        "../../vendor/vector-ta-0.2.9-patched/src/indicators/moving_averages/elastic_volume_weighted_moving_average.rs",
    );
    assert!(evwma.contains(
        "evwma_rolling_volume_close_length_key_default30_chronological_rn_f64_v1"
    ));
    assert!(evwma.contains("EVWMA_FIXED_GAMMA_N_F64_AUTHORITY_V1"));

    let default_candles = function_body(&evwma, "pub fn with_default_candles(");
    assert!(default_candles.contains("\"close\""));
    assert!(!default_candles.contains("\"hlcc4\""));
    let builder_apply = function_body(&evwma, "pub fn apply(");
    assert!(builder_apply.contains("\"close\""));
    assert!(!builder_apply.contains("\"hlcc4\""));

    let fixed_update = function_body(&evwma, "fn evwma_fixed_gamma_n_update_f64_v1(");
    assert!(fixed_update.contains("volume * inv_absolute_volume"));
    assert!(fixed_update.contains("base + weight * (price - base)"));
    let absolute = function_body(&evwma, "fn compute_absolute_into(");
    let stream = function_body(&evwma, "pub fn update(");
    assert!(absolute.contains("evwma_fixed_gamma_n_update_f64_v1"));
    assert!(stream.contains("evwma_fixed_gamma_n_update_f64_v1"));

    let cpu_dispatch = source(
        "../../vendor/vector-ta-0.2.9-patched/src/indicators/dispatch/cpu_batch.rs",
    );
    let period = function_body(&cpu_dispatch, "fn ma_period_for_combo(");
    assert!(period.contains("elastic_volume_weighted_moving_average"));
    assert!(period.contains("\"length\""));
    let production_dispatch = function_body(&cpu_dispatch, "fn compute_ma_batch(");
    assert!(production_dispatch.contains("validate_production_evwma_params"));
    let production_mode =
        function_body(&cpu_dispatch, "fn validate_production_evwma_params(");
    assert!(production_mode.contains("Some(ParamValue::Bool(false))"));
    assert!(production_mode.contains("use the dedicated EVWMA API for fixed gamma N"));

    let cuda_dispatch = source(
        "../../vendor/vector-ta-0.2.9-patched/src/indicators/dispatch/cuda_f64.rs",
    );
    let marker = "indicator_id: \"elastic_volume_weighted_moving_average\"";
    let marker_offset = cuda_dispatch.find(marker).expect("EVWMA f64 row");
    let row_start = cuda_dispatch[..marker_offset]
        .rfind("F64KernelSpec {")
        .expect("EVWMA f64 row start");
    let row_end = cuda_dispatch[marker_offset..]
        .find("},")
        .map(|offset| marker_offset + offset)
        .expect("EVWMA f64 row end");
    let row = &cuda_dispatch[row_start..row_end];
    assert!(row.contains("input: F64InputKind::CloseVolume"));
    assert!(!row.contains("input: F64InputKind::Hlcc4Volume"));

    let gpu_dispatch = source("src/core/gpu_indicators.rs");
    let finite_pair = function_body(
        &gpu_dispatch,
        "fn first_valid_price_volume_finite_for_input(",
    );
    assert!(finite_pair.contains("DeviceInput::CloseVolume"));
    assert!(finite_pair.contains("DeviceInput::Hlcc4CloseVolume"));
    let first_valid = function_body(&gpu_dispatch, "fn first_valid_for(");
    assert!(first_valid.contains("first_valid_close_volume_finite"));
    assert!(!first_valid.contains(
    ...
    let registry = source("src/core/feature_registry.rs");
    assert!(registry.contains("CLASSIC_VECTOR_TA_SEMANTIC_VERSION_V8: u32 = 8"));
    assert!(registry.contains("evwma/rolling-volume-sum/close/f64/v1"));
    assert!(registry.contains("evwma/rolling-length-key/length/default-30/f64/v1"));
    assert!(registry.contains("evwma/fixed-n/close/f64/v1"));
    assert!(registry.contains("refuse semantic-v7 and unversioned ClassicVectorTa artifacts"));
    for path in [
        "vendor/vector-ta-0.2.9-patched/src/indicators/moving_averages/elastic_volume_weighted_moving_average.rs",
        "vendor/vector-ta-0.2.9-patched/src/indicators/dispatch/cuda_f64.rs",
        "vendor/vector-ta-0.2.9-patched/kernels/cuda/moving_averages/elastic_volume_weighted_moving_average_kernel.cu",
        ...
        assert_eq!(
            ...
            1,
            "ClassicVectorTa v8 must bind `{path}` exactly once"
        );
    }
}
