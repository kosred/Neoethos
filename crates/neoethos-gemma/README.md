...
- The trait surface lets the ensemble integrate via a one-way
  dependency (`neoethos-gemma → neoethos-models`, never the reverse).
...
`neoethos-gemma` είναι το νέο crate για το local Gemma-4-E4B
...
Default OFF στο neoethos-app μέσω του `gemma-helper` feature.
...
4. `%LOCALAPPDATA%\neoethos\models\<filename>` (Windows) /
   `$HOME/.neoethos/models/<filename>` (POSIX) — user-data swap-in.
