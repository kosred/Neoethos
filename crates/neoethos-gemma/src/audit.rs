...
//! `neoethos_core::SchemaVersion` + `HasSchemaVersion` so the audit
...
/// file at `~/.neoethos/gemma_audit.jsonl`; G0 ships only the
...
mod tests {
    ...
    fn topic_check_lifts_into_audit_verdict_without_canned_text() {
        let check = TopicCheck::Refuse {
            ...
            canned_response: "I can only help with neoethos…".to_string(),
        };
...
mod jsonl_tests {
    ...
    fn temp_path(name: &str) -> std::path::PathBuf {
        ...
        let nonce = std::time::SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        p.push(format!("neoethos-gemma-audit-{name}-{nonce}.jsonl"));
        p
    }
    ...
    fn jsonl_creates_parent_directories_on_demand() {
        ...
        let nonce = std::time::SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        p.push(format!("neoethos-gemma-audit-mkdirp-{nonce}"));
        p.push("subdir");
        p.push("audit.jsonl");
