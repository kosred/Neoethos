//! Error type for the neoethos-gemma helper crate.
//!
...
pub enum GemmaError {
    ...
    /// `neoethos_core::SchemaVersionError` semantics.
    ...
    /// often hit when `neoethos-app` is built without the
...
mod tests {
    ...
    fn topic_gate_refused_keeps_canned_response_for_caller() {
        let err = GemmaError::TopicGateRefused {
            ...
            canned_response: "Μπορώ να βοηθήσω μόνο με ερωτήσεις σχετικά με το neoethos."
                .to_string(),
        };
