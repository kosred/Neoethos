...
//!
//! `neoethos_models::ensemble_inference::ExpertModel`:
//!   - name() -> &str    ("gemma_e4b")
...
impl GemmaExpertInferenceAdapter for StubGemmaExpertInferenceAdapter {
    fn infer(&self, _prompt_payload: &str) -> Result<Option<GemmaRawVote>, GemmaError> {
        Err(GemmaError::pending(
            "G6 GemmaExpert ensemble adapter (neoethos-models integration)",
        ))
    }
}
...
// ---------------------------------------------------------------------------
// G6a — `GemmaExpert` scaffolding (neoethos-models dep deferred behind feature)
// ---------------------------------------------------------------------------

/// G6a feature gate. Enabling pulls in `neoethos-models` and turns
...
///
/// Why not just import `neoethos-models` unconditionally? Two
/// reasons:
/// - `neoethos-models` pulls candle + Burn + linfa + a hefty
///   compile-time payload. The bare `neoethos-gemma` crate stays
///   ~10s to compile in CI; with `neoethos-models` it climbs to
///   minutes.
/// - The two-way dep `neoethos-gemma → neoethos-models` is fine but
///   we want the reverse direction (a model adapter pulling
///   into `neoethos-models` for the registry) to land in a single
...
/// without pulling in `neoethos-models`. The G6b commit will land:
///
/// ```ignore
/// #[cfg(feature = "ensemble-integration")]
/// impl neoethos_models::ensemble_inference::ExpertModel for GemmaExpert { ... }
/// ```
///
