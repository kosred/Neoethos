...
pub fn refusal_text(language: LanguageHint) -> String {
    match language {
        LanguageHint::Greek => "Μπορώ να βοηθήσω μόνο με ερωτήσεις σχετικά με το \
             neoethos — το bot, τα trades, τα μοντέλα ή τις θέσεις σας."
            .to_string(),
        // English / Unknown share the English text.
        _ => "I can only help with neoethos — its bot, trading, models, broker \
...
mod tests {
    ...
    fn ignore_previous_instructions_is_refused_english() {
        ...
        {
            assert!(canned_response.contains("neoethos"));
        }
    }
    ...
    fn refusal_text_picks_greek_for_greek_hint() {
        ...
        assert!(
            txt.contains("neoethos") && txt.contains("Μπορώ"),
            "Greek refusal must be in Greek and mention neoethos: {txt}"
        );
    }
