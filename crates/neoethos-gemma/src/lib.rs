...
// ── Bundled model constants ─────────────────────────────────────────────
//
// The CANONICAL definitions live in `runtime.rs` (alongside the lookup
// chain that consumes them). We re-export them here so external callers
// can write `neoethos_gemma::BUNDLED_MODEL_FILENAME` without spelling
// `::runtime::` — and, more importantly, so there is exactly ONE place
// where the file name / URL / size live. Earlier drift between this
// module and `runtime.rs` (Gemma 3 vs Gemma 4) is why this is a re-export
// now rather than a duplicate.
