...
//! 4. `<dirs::data_dir>/neoethos/models/*.gguf` — XDG-style
...
impl FsProbe for RealFsProbe {
    ...
    fn user_data_dir(&self) -> Option<PathBuf> {
        ...
        // back to `$HOME/.neoethos/models` (POSIX) /
        // `%LOCALAPPDATA%\neoethos\models` (Windows).
        if cfg!(windows) {
            std::env::var("LOCALAPPDATA")
                .ok()
                .map(|s| PathBuf::from(s).join("neoethos"))
        } else {
            std::env::var("HOME")
                .ok()
                .map(|s| PathBuf::from(s).join(".neoethos"))
...
mod llama_impl {
    ...
    impl LlamaCppGemmaRuntime {
        ...
        pub fn load(model_path: PathBuf) -> Result<Self> {
            ...
            std::thread::Builder::new()
                .name("neoethos-gemma-llama".to_string())
                .spawn(move || inference_worker(model_path, rx))
                .context("failed to spawn neoethos-gemma inference thread")?;
    ...
    fn inference_worker(model_path: PathBuf, rx: mpsc::Receiver<InferRequest>) {
        ...
        tracing::debug!(
            target: "neoethos_gemma::runtime",
            "neoethos-gemma inference worker: channel closed, exiting"
        );
    }
...
mod tests {
    ...
    //   cargo test -p neoethos-gemma --features mistralrs-runtime
    #[cfg(feature = "mistralrs-runtime")]
    #[test]
    ...
    fn resolver_falls_through_to_user_data_dir() {
        let probe = FakeFsProbe::new()
            .with_exe("/app")
            .with_manifest("/repo")
            .with_user("/home/neoethos")
            .add_file(PathBuf::from("/home/neoethos/models").join(BUNDLED_MODEL_FILENAME));
        let r = resolve_bundled_model_path(&probe).unwrap();
