...
mod tests {
    ...
    fn expected_training_backend_kind() -> neoethos_core::BackendKind {
        #[cfg(feature = "neuro-evolution")]
        {
            neoethos_core::BackendKind::NativeCpu
        }
        #[cfg(not(feature = "neuro-evolution"))]
        {
            neoethos_core::BackendKind::LocalSurrogateFallback
        }
    }
    ...
    fn neuro_evo_predict_runtime_reports_truthful_fallback_backend() -> Result<()> {
        ...
        assert_eq!(
            ...
            Some(neoethos_core::BackendKind::LocalSurrogateFallback)
        );
        assert_eq!(
            predictions[0].metadata().runtime_mode,
            Some(neoethos_core::RuntimeMode::Degraded)
        );
        assert_eq!(
