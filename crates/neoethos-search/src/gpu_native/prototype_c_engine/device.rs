...
fn emit_c_population_events(
    close_pips: &[f32],
    adaptive_base_pips: &[f32],
    stop_pips: &[f32],
    target_pips: &[f32],
    stop_vol_multipliers: &[f32],
    signals: &[i32],
    event_offsets: &[i32],
    event_candidate_out: &mut [i32],
    event_scenario_out: &mut [i32],
    event_entry_out: &mut [i32],
    event_last_out: &mut [i32],
    event_direction_out: &mut [i32],
    event_stop_out: &mut [f32],
    event_target_out: &mut [f32],
    n_bars: u32,
    population: u32,
    max_hold_bars: u32,
    min_hold_bars: u32,
...
fn c_population_first_hit_kernel(
    ...
    event_stop: &[f32],
    event_target: &[f32],
    exit_bar_out: &mut [i32],
    exit_reason_out: &mut [i32],
    event_count: u32,
    n_bars: u32,
    min_hold_bars: u32,
    max_hold_bars: u32,
...
fn stitch_c_population_trades(
    ...
    exit_bar: &[i32],
    exit_reason: &[i32],
    accepted_event_out: &mut [i32],
    accepted_count_out: &mut [i32],
    n_bars: u32,
    population: u32,
    ...
    fn submit_population_chain(&mut self) -> Result<usize, EngineError> {
...

        {
            ...
            c_population_signals_kernel::launch::<R>(
                ...
                signal_dim,
                unsafe {
                    BufferArg::from_raw_parts(dataset.indicators.clone(), dataset.indicator_len)
                },
                unsafe { BufferArg::from_raw_parts(genes.offsets.clone(), population + 1) },
                unsafe { BufferArg::from_raw_parts(genes.indices.clone(), genes.term_len.max(1)) },
                unsafe { BufferArg::from_raw_parts(genes.weights.clone(), genes.term_len.max(1)) },
                unsafe { BufferArg::from_raw_parts(genes.long_thresholds.clone(), population) },
                unsafe { BufferArg::from_raw_parts(genes.short_thresholds.clone(), population) },
                unsafe { BufferArg::from_raw_parts(dataset.smc_rows.clone(), bars * SMC_WIDTH) },
                unsafe {
                    BufferArg::from_raw_parts(genes.smc_flags.clone(), population * SMC_WIDTH)
                },
                unsafe { BufferArg::from_raw_parts(genes.smc_weights.clone(), SMC_WIDTH) },
                unsafe { BufferArg::from_raw_parts(genes.signals.clone(), genes.signal_len) },
                unsafe { BufferArg::from_raw_parts(genes.confidences.clone(), genes.signal_len) },
                bars as u32,
                population as u32,
            ...
            c_population_count_events_kernel::launch::<R>(
                ...
                count_dim,
                unsafe { BufferArg::from_raw_parts(genes.signals.clone(), genes.signal_len) },
                unsafe { BufferArg::from_raw_parts(genes.event_counts.clone(), population) },
                bars as u32,
                population as u32,
            );
            ...
            c_population_scan_offsets_kernel::launch::<R>(
                ...
                CubeDim::new_1d(1),
                unsafe { BufferArg::from_raw_parts(genes.event_counts.clone(), population) },
                unsafe { BufferArg::from_raw_parts(genes.event_offsets.clone(), population + 1) },
                unsafe { BufferArg::from_raw_parts(genes.event_total.clone(), 1) },
                population as u32,
            );
        }
...

        {
            ...
            emit_c_population_events::launch::<R>(
                ...
                emit_cubes,
                emit_dim,
                unsafe { BufferArg::from_raw_parts(dataset.close.clone(), bars) },
                unsafe {
                ...
                unsafe { BufferArg::from_raw_parts(genes.stop_pips.clone(), population) },
                unsafe { BufferArg::from_raw_parts(genes.target_pips.clone(), population) },
                unsafe {
                    BufferArg::from_raw_parts(genes.stop_vol_multipliers.clone(), population)
                },
                unsafe { BufferArg::from_raw_parts(genes.signals.clone(), genes.signal_len) },
                unsafe { BufferArg::from_raw_parts(genes.event_offsets.clone(), population + 1) },
                unsafe { BufferArg::from_raw_parts(events.candidate.clone(), capacity) },
                unsafe { BufferArg::from_raw_parts(events.scenario.clone(), capacity) },
                unsafe { BufferArg::from_raw_parts(events.entry.clone(), capacity) },
                unsafe { BufferArg::from_raw_parts(events.last.clone(), capacity) },
                unsafe { BufferArg::from_raw_parts(events.direction.clone(), capacity) },
                unsafe { BufferArg::from_raw_parts(events.stop.clone(), capacity) },
                unsafe { BufferArg::from_raw_parts(events.target.clone(), capacity) },
                bars as u32,
                population as u32,
            ...
            if emitted > 0 {
                ...
                c_population_first_hit_kernel::launch::<R>(
                    ...
                    hit_cubes,
                    hit_dim,
                    unsafe { BufferArg::from_raw_parts(dataset.high.clone(), bars) },
                    unsafe { BufferArg::from_raw_parts(dataset.low.clone(), bars) },
                    unsafe { BufferArg::from_raw_parts(dataset.gap_flags.clone(), bars) },
                    unsafe { BufferArg::from_raw_parts(events.entry.clone(), capacity) },
                    unsafe { BufferArg::from_raw_parts(events.last.clone(), capacity) },
                    unsafe { BufferArg::from_raw_parts(events.direction.clone(), capacity) },
                    unsafe { BufferArg::from_raw_parts(events.stop.clone(), capacity) },
                    unsafe { BufferArg::from_raw_parts(events.target.clone(), capacity) },
                    unsafe { BufferArg::from_raw_parts(events.exit_bar.clone(), capacity) },
                    unsafe { BufferArg::from_raw_parts(events.exit_reason.clone(), capacity) },
                    emitted as u32,
                    bars as u32,
                    settings.min_hold_bars as u32,
                    settings.max_hold_bars as u32,
            ...
            stitch_c_population_trades::launch::<R>(
                ...
                stitch_cubes,
                stitch_dim,
                unsafe { BufferArg::from_raw_parts(dataset.days.clone(), bars) },
                unsafe { BufferArg::from_raw_parts(genes.event_offsets.clone(), population + 1) },
                unsafe { BufferArg::from_raw_parts(events.entry.clone(), capacity) },
                unsafe { BufferArg::from_raw_parts(events.exit_bar.clone(), capacity) },
                unsafe { BufferArg::from_raw_parts(events.exit_reason.clone(), capacity) },
                unsafe { BufferArg::from_raw_parts(events.accepted_event.clone(), capacity) },
                unsafe { BufferArg::from_raw_parts(genes.accepted_count.clone(), population) },
                bars as u32,
                population as u32,
            ...
            reduce_c_population_metrics::launch::<R>(
                ...
                unsafe { BufferArg::from_raw_parts(dataset.high.clone(), bars) },
                unsafe { BufferArg::from_raw_parts(dataset.low.clone(), bars) },
                unsafe { BufferArg::from_raw_parts(dataset.months.clone(), bars) },
                unsafe { BufferArg::from_raw_parts(dataset.days.clone(), bars) },
                unsafe { BufferArg::from_raw_parts(dataset.timestamp_pair.clone(), bars * 2) },
                unsafe { BufferArg::from_raw_parts(genes.confidences.clone(), genes.signal_len) },
                unsafe { BufferArg::from_raw_parts(events.entry.clone(), capacity) },
                unsafe { BufferArg::from_raw_parts(events.direction.clone(), capacity) },
                unsafe { BufferArg::from_raw_parts(events.stop.clone(), capacity) },
                ...
                unsafe { BufferArg::from_raw_parts(genes.metrics.clone(), genes.metrics_len) },
                bars as u32,
                population as u32,
