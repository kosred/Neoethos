...
fn launch_sparse_first_hit<R: Runtime>(
    ...
    sparse_first_hit_kernel::launch::<R>(
        ...
        unsafe { BufferArg::from_raw_parts(highs_handle, highs.len()) },
        unsafe { BufferArg::from_raw_parts(lows_handle, lows.len()) },
        unsafe { BufferArg::from_raw_parts(entry_handle, events.len()) },
        unsafe { BufferArg::from_raw_parts(last_handle, events.len()) },
        unsafe { BufferArg::from_raw_parts(direction_handle, events.len()) },
        unsafe { BufferArg::from_raw_parts(stop_handle, events.len()) },
        unsafe { BufferArg::from_raw_parts(target_handle, events.len()) },
        unsafe { BufferArg::from_raw_parts(precedence_handle, events.len()) },
        unsafe { BufferArg::from_raw_parts(exit_bar_handle.clone(), events.len()) },
        unsafe { BufferArg::from_raw_parts(exit_reason_handle.clone(), events.len()) },
        events.len() as u32,
    );
