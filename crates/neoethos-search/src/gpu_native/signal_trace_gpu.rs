...
fn signal_trace_kernel(
    ...
    gene_smc_flags: &[i32],
    smc_weights: &[f32],
    scores_before_threshold: &mut [f32],
    raw_signals: &mut [i32],
    confidences: &mut [f32],
    smc_scores: &mut [f32],
    smc_gates: &mut [f32],
    final_signals: &mut [i32],
    sample_count: u32,
    gate_threshold: f32,
) {
...
fn launch_signal_trace<R: Runtime>(
    ...
    signal_trace_kernel::launch::<R>(
        ...
        unsafe { BufferArg::from_raw_parts(indicators, input.indicators_feature_major.len()) },
        unsafe { BufferArg::from_raw_parts(offsets, input.gene_offsets.len()) },
        unsafe { BufferArg::from_raw_parts(indices, input.gene_indices.len()) },
        unsafe { BufferArg::from_raw_parts(weights, input.gene_weights.len()) },
        unsafe { BufferArg::from_raw_parts(long, input.gene_count()) },
        unsafe { BufferArg::from_raw_parts(short, input.gene_count()) },
        unsafe { BufferArg::from_raw_parts(smc, smc_data.len()) },
        unsafe { BufferArg::from_raw_parts(flag_handle, flags.len()) },
        unsafe { BufferArg::from_raw_parts(smc_weights, SMC_WIDTH) },
        unsafe { BufferArg::from_raw_parts(score_out.clone(), total) },
        unsafe { BufferArg::from_raw_parts(raw_out.clone(), total) },
        unsafe { BufferArg::from_raw_parts(confidence_out.clone(), total) },
        unsafe { BufferArg::from_raw_parts(smc_score_out.clone(), total) },
        unsafe { BufferArg::from_raw_parts(smc_gate_out.clone(), total) },
        unsafe { BufferArg::from_raw_parts(final_out.clone(), total) },
