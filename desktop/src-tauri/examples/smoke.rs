...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let has_token = token
        ...
        .unwrap_or(false);
    let refresh_present = token
        .as_ref()
        .map(|b| !b.refresh_token.is_empty())
        .unwrap_or(false);
    println!(
    ...
    match broker_api::fetch_broker_symbols_blocking() {
        Ok(b) => println!(
            "[broker_symbols]  OK  env={}  {} symbols (account {})",
            b.environment,
            b.symbols.len(),
            b.account_id
        ),
    ...
    match broker_api::fetch_account_runtime_blocking() {
        Ok(snap) => {
            println!(
                ...
                snap.trader.balance,
                snap.unrealized_pnl,
                snap.trader.balance + snap.unrealized_pnl,
            ...
            for p in snap.reconcile.positions.iter().take(5) {
                println!(
                    "    position #{} sym#{} {} vol={} @ {:?}",
                    p.position_id, p.symbol_id, p.trade_side, p.volume, p.price
                );
            }
        }
        Err(e) => println!(
            "[account_snapshot]  ERR (expected if refresh_token revoked → one-time OAuth needed)  {e}"
        ),
    }
    ...
    match broker_api::fetch_broker_accounts_blocking() {
        Ok(b) => {
            println!(
                "[broker_accounts]  OK  {} accounts (scope: {})",
                b.accounts.len(),
                b.permission_scope
            );
    ...
    match broker_api::fetch_recent_chart_bars_blocking("EURUSD", "H1", 50) {
        Ok(bars) => println!(
            "[broker_chart]  OK  {} broker bars  last_close={:?}",
            bars.len(),
