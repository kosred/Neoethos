...
mod backend {
    ...
    /// True when any engine is queued or running, including Native Research.
    ...
    pub fn engine_running() -> bool {
        ...
        // Native Research owns its lease while Queued too; closing in that
        // pre-launch window would otherwise drop the app-owned worker early.
        body.contains("\"Running") || body.contains("\"Queued")
    }
}
