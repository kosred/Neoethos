...
const SECTIONS: Sec[] = [
  ...
  { id: "cockpit", icon: "🎯", title: "Trade (Cockpit)", group: "Trade",
    what: "The all-in-one trading desk: a market watch list, a live chart, an order ticket and full open-position management on one screen.",
    how: [
      ...
      "Use the order ticket (Lots + optional SL/TP in pips) to place a manual BUY/SELL on the active account.",
      "Open positions show live P/L at the bottom with one-click close.",
      "Click Edit on a position to set SL/TP as price levels (e.g. move a stop to breakeven) or enable trailing.",
  ...
  { id: "account", icon: "💳", title: "Account & Journal", group: "Trade",
    what: "One screen for everything account-shaped: the closed-trade journal with computed stats (MyFxbook-style), plus broker identity, order history, cash flow and a margin calculator.",
    how: [
      "Journal tab: win rate, profit factor and every closed trade — fills automatically as trades close on the account.",
      "Broker & history tab: raw order history, deposits/withdrawals/swaps and the margin calculator.",
    ] },
  ...
  { id: "aidesk", icon: "💬", title: "AI Desk", group: "Desk",
    ...
    how: [
      "Type the ChatGPT account email you want to use in the 'ChatGPT email' box (optional — leave blank to pick it in the browser), then Connect.",
      "You sign in on ChatGPT's own page — NeoEthos never sees your password; the email only tells it which account to use.",
      "Use Switch account any time to connect a different ChatGPT account.",
      "Once connected, type a message and Send.",
    ] },
  // ── System ──
  ...
  { id: "settings", icon: "⚙", title: "Settings", group: "System",
    what: "The single place to change configuration — discovery mode, risky goal, search tuning, compute, risk, news, broker — plus the power-user Advanced tools (diagnostics, data import, presets, raw YAML) further down the same screen. Each control writes config.yaml; only Discovery + Training actions live outside.",
    how: [
