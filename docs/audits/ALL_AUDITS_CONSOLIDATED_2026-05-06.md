...
Phases 63-65 landed three shared modules under `forex-core::utils`:

- **Phase 63 — `hashing`**: `fnv1a64` / `fnv1a64_update` with the canonical FNV-1a constants. Three duplicate sites (`forex-core::contracts::temporal`, `forex-search::artifact_io`, `forex-search::genetic::evolution_math`) switched to the shared helper. Backward-compat shims kept on `artifact_io::fnv1a64` and `evolution_math::fnv1a_update` because both are call-site ergonomics, not duplicate constants.
- **Phase 64 — `stats`**: `mean`, `stddev` (population), `stddev_sample` (Bessel), `mean_std` (NaN-tolerant), `pearson_correlation_f32`, `mean_vector_f32`. Five duplicate sites in `forex-search` (portfolio, quality, stop_target, eval, cubecl_eval) switched. Tests cover known-vector sanity (population vs sample stddev, NaN drop in mean_std, perfect-correlation cases for Pearson, mean_vector_f32 mismatched-length skip).
- **Phase 65 — `numeric`**: `finite_or`, `finite_or_f32`, `clamp_unit_f32` / `_f64`, `stable_sigmoid_f32` (handles overflow at ±50 logits). Five sites switched (`genetic::diversity`, `genetic::regime_labels`, `ensemble::sigmoid`, `bayesian_impl::sigmoid`, `forecasting::swarm_impl::clamp_unit`). The naive `1/(1+e^{-x})` form on `ensemble.rs` was the worst offender; it now routes through the stable form.

Phase 66 documents the closure: forex-core lib tests grew 51 → 62 (+11 from the new utility modules), forex-search lib tests stay at 107, downstream `cargo check` on forex-cli / forex-app / forex-models remains clean. Out-of-scope deferrals (`atomic_io` lift across forex-search ↔ forex-models, `flatten_features` cuda_prep extraction, OHLCV slicing on forex-data) are listed in [`dedup_findings_2026-05-10.md`](dedup_findings_2026-05-10.md) with the reason each was held back — all are pure code-organisation work that can land later without touching contract surfaces.

The user's directive to "dig further and mostly in the master branch" is satisfied: the broader scan in Phase 62 covered every crate outside the 16-60 work surface, and the three landed modules cover the highest-frequency duplicate clusters across forex-core / forex-search / forex-models. The audit follow-on slice that began at Phase 16 is complete at Phase 66.


### 2026-05-10: Follow-on Phase 61 completed — duplicate-function audit + extraction plan

Audit pass over the repo to surface duplicated private helpers and parallel implementations that survived Phases 1-60. Findings are documented in [`dedup_findings_2026-05-10.md`](dedup_findings_2026-05-10.md) grouped into nine categories: FNV hash helpers (3 copies across forex-core / forex-search), atomic JSON IO (5 sites split between forex-search / forex-models / forex-data), `mean` / `stddev` / `mean_std` (5 sites in forex-search), Pearson correlation f32/i8 in the same file, `mean_vector` (2 GPU files), `finite_or` (2 genetic files), `clamp_probability` / `clamp_unit` / `clamp_f32` (4 forex-models files), env-var readers (3 different shapes, one is the canonical typed boundary from Phases 17-22), and OHLCV slicing (2 signatures). Out-of-scope reminders explicitly call out signal-synthesis variants and the population vs scalar evaluator as look-alike-but-not-duplicate code. Proposed module layout puts the shared helpers under `forex-core::utils::{hashing, stats, numeric, env, atomic_io}` and `forex-data::slicing`. Recommended sequencing assigns Phases 62-66 to the extraction work, with the existing testing discipline (small diff + tests around the change + `cargo fmt/test/check` on every downstream crate before commit). Closes the user-requested dedup audit; extraction phases will land in a separate session.
