...
- `FIXME(hardcoded)`: 24 matches remain in Rust files after shared challenge/runtime defaults were extracted in this branch (37 at initial audit).
...
| Hardcoded risk/config values | Roadmap says 37 `FIXME(hardcoded)` remain and mostly need config extraction. | Initial matches included duplicate challenge windows, relaxed validation min-days, runtime risk bands, broker lot ceiling, trade caps, and strategy tunables. This branch adds `PropFirmChallengeDefaults` and `PropFirmRuntimeDefaults` in `forex-core::domain::prop_firm`, then wires core risk presets plus search challenge/validation defaults to that shared boundary. 24 `FIXME(hardcoded)` remain: default config strategy tunables, phase-specific risk/confidence/trade caps, recovery bands, and consistency ratio. | **Partially fixed in this branch** | Continue with one remaining tunable family at a time only when tests can lock behavior; do not broad-refactor config shape. |
...
6. Shared challenge/runtime defaults now remove the cross-crate challenge-window duplication; triage the remaining 24 hardcoded strategy/recovery tunables only with focused tests.
