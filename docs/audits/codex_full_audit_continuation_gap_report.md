...
- `FIXME(hardcoded)`: 22 matches remain in Rust files after shared challenge/runtime/recovery defaults were extracted in this branch (37 at initial audit).
...
| Hardcoded risk/config values | Roadmap says 37 `FIXME(hardcoded)` remain and mostly need config extraction. | Initial matches included duplicate challenge windows, relaxed validation min-days, runtime risk bands, broker lot ceiling, trade caps, recovery bands, and strategy tunables. This branch adds `PropFirmChallengeDefaults` and `PropFirmRuntimeDefaults` in `forex-core::domain::prop_firm`, then wires core risk presets plus search challenge/validation defaults to that shared boundary. Recovery drawdown bands/multipliers now share the same runtime defaults in both the order gate and position sizing. 22 `FIXME(hardcoded)` remain: default config strategy tunables, phase-specific risk/confidence/trade caps, phase profit-lock thresholds, and consistency ratio. | **Partially fixed in this branch** | Continue with one remaining tunable family at a time only when tests can lock behavior; do not broad-refactor config shape. |
...
5. cTrader TLS provider ambiguity and the pre-close unit-test live-transport leak are fixed; keep future live broker checks in explicit integration tests.
6. Shared challenge/runtime/recovery defaults now remove the cross-crate challenge-window duplication and the duplicated recovery bands; triage the remaining 22 hardcoded strategy tunables only with focused tests.
