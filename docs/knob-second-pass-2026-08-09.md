...
| `system.enable_gpu` | `:165` | the probe | The former `rllib_auto` reader was removed 2026-08-25 with the fail-closed RLlib migration boundary. It now has no production reader; retire it through the sealed-loader tombstone path rather than restoring the fallback branch |
