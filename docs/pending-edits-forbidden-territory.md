...
| W2-20 | **DISCHARGED 2026-08-10 (audit #294)** | `crates/neoethos-core/tests/config_has_recipient.rs` | Four places still cite `AutoTuneHints` as the reason a knob has no recipient — the rows for `ModelsConfig::inference_batch_size` and `SystemConfig::n_jobs`, and two prose justifications quoting `system.rs:1251` / `:1256`. Those lines no longer exist and the type is deleted. The only in-code occurrence is inside a string literal in a parser unit test, which still exercises the parser unchanged. | The two ledger rows are GONE, because the two FIELDS are gone: `SystemConfig::n_jobs`, `SystemConfig::num_gpus` and `ModelsConfig::inference_batch_size` were deleted from the structs and their `NO_QUALIFIED_READER` entries deleted in the same change (the ledger is checked both ways, so a stale entry naming a removed field fails the build). The keys stay in `load_seal::RETIRED_KEYS` as `Derived`, so the operator's existing store still loads with each one named at WARN. The only remaining occurrences are inside string literals in the parser unit tests, which exercise the parser unchanged. |
...
## ~~`models.blend_*` — the live blend multipliers need a config recipient~~ — LANDED 2026-08-10 (audit #232)

> **CLOSED.** The full drag below was executed, so nothing here is outstanding.
> It is kept as the record of what was done, not as work:
>
> 1. `crates/neoethos-core/src/config.rs` — `ModelsConfig::blend_gate_floor`
>    (0.34) and `blend_veto_below` (0.15), with the two `Default` entries.
> 2. repo `config.yaml` — **deliberately NOT touched.** That file was collapsed
>    to OVERRIDES ONLY (`the_repo_profile_carries_only_its_overrides`); a key
>    whose value equals the compiled default is deleted from it, so adding
>    default-equal lines would fail that test. This point of the drag predates
>    the collapse.
> 3. `desktop/src-tauri/resources/config.yaml` — the two lines, in struct
>    declaration order beside `live_ml_gate`. That file is GENERATED; re-run
>    `cargo test -p neoethos-core --test generated_seed_is_current` to confirm.
> 4. `crates/neoethos-app/src/server/settings.rs` — response struct, patch
>    struct, apply arm (range clamp + non-finite refusal), read-back.
> 5. `crates/neoethos-app/src/server/knob_catalog.rs` — one row each under
>    "Risk & PnL safety", reading the operator's current value.
> 6. `crates/neoethos-mcp/src/params.rs` + `ops.rs` — mirrored into the control
>    plane as `blendGateFloor` / `blendVetoBelow`.
>
> The two waiting readers `operator_blend_gate_floor` / `_veto_below`
> (`live_trading.rs`) are one-liners now and return the operator's number.
> Regression: `the_operator_blend_multipliers_reach_the_live_blend_config`
> (`live_trading.rs` test module) pins the configured pair reaching
> `BlendConfig`, the `None` case landing on the shipped defaults, and an
> inverted pair being refused back to them.
