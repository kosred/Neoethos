...
- the `events` trail is now structured with explicit levels (`Info`, `Warning`, `Error`) and is rendered with level-aware UI styling, so operator-facing milestones are no longer plain untyped strings
