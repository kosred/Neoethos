...
impl<'a> Command<'a> {
    ...
    /// Explicitly clean GPU and pinned-host memory on the current stream.
    pub fn memory_cleanup(&mut self) {
        ...
        // slices as live. The queue is double-buffered: one flush only rotates
        // the current batch, hence two are required before cleaning the pools.
        let sys = stream.sys;
