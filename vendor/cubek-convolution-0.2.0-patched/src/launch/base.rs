...
use cubecl::{Runtime, client::ComputeClient, features::Tma};
use cubek_matmul::{
    components::tile::TileMatmulKind,
    definition::{MatmulAvailabilityError, MatmulElems, TilingBlueprint},
...
pub fn launch_ref<R: Runtime, const N_SPATIAL: usize>(
    ...
    if matches!(
        algorithm,
        ConvAlgorithm::SimpleAsyncTma | ConvAlgorithm::SpecializedTma
    ) && !client.properties().features.tma.contains(Tma::Base)
    {
