...
use lightgbm3_sys::{
    C_API_DTYPE_FLOAT32, C_API_DTYPE_FLOAT64, DatasetHandle, LGBM_DatasetGetFeatureNames,
...
impl Dataset {
    ...
    pub fn from_slice<T: DType>(
        ...
        Self::from_slice_with_reference_and_params(
            flat_x,
            label,
            n_features,
            is_row_major,
    ...
    /// Creates a dataset with the same native parameters that will be passed
    /// to the booster.
    ///
    /// This is mandatory for `device_type=cuda`: LightGBM allocates CUDA
    /// column storage and metadata while finishing the dataset, before the
    /// booster and CUDA objective exist. Passing CUDA only to `Booster::train`
    /// leaves CPU dataset metadata behind and the native CUDA objective cannot
    /// safely initialize from it.
    ...
    /// Parameterized variant of [`Self::from_slice_with_reference`].
    pub fn from_slice_with_reference_and_params<T: DType>(
        flat_x: &[T],
        label: &[f32],
        n_features: i32,
        is_row_major: bool,
        ...
        let params = parameters
            .as_object()
            .ok_or_else(|| Error::new("LightGBM dataset parameters must be a JSON object"))?
            .iter()
            .map(|(key, value)| format!("{key}={value}"))
            .collect::<Vec<_>>()
            .join(" ");
        let params = CString::new(params)
            .map_err(|error| Error::new(format!("invalid LightGBM dataset parameters: {error}")))?;
