...
VectorTA is a Rust crate of 340 implemented technical analysis indicators focused on speed, predictable allocations, and practical execution flexibility, with optional SIMD/CUDA acceleration. In addition to standard single call APIs, much of the library also supports streaming/stateful updates, batch parameter sweeps, and registry-driven dispatch across multiple input and output shapes.

It is intended for workloads where throughput and execution behavior actually matter, including research pipelines, backtesting systems, and high throughput production use cases. The library is not limited to close only, single output indicators either. It spans a wide range of input structures, multi-output studies, and execution paths across Rust CPU and CUDA runtimes.

The CUDA path is predominantly only worth using in a VRAM-resident workflow. For example, it is possible to achieve a benchmark timing of 3.129 ms for 250 million calculated ALMA indicator data points on an RTX 4090, whereas the CPU (AMD 9950X) AVX-512, AVX2, and scalar timings are approximately 140.61 ms, 188.64 ms, and 386.20 ms, respectively.
...
- `cuda-build-native`: compile and embed one verified native cubin per CUDA
  kernel and exact requested or visible GPU architecture. This feature also
  enables the Rust CUDA API. Missing exact architectures fail closed.
...
```toml
[dependencies]
vector-ta = {version = "0.2.9", features = ["cuda-build-native"]}
```
