...
use vector_ta::indicators::moving_averages::{
    alma::{AlmaBatchBuilder, AlmaInput, alma_with_kernel},
    buff_averages::{
        BuffAveragesBatchBuilder, BuffAveragesInput, buff_averages, buff_averages_with_kernel,
    },
    cwma::{CwmaBatchBuilder, CwmaInput, cwma_with_kernel},
    dema::{DemaBatchBuilder, DemaInput, dema_with_kernel},
    dma::{DmaBatchBuilder, DmaInput, dma_with_kernel},
    edcf::{EdcfBatchBuilder, EdcfInput, edcf_with_kernel},
    ehlers_ecema::{
        EhlersEcemaBatchBuilder, EhlersEcemaInput, ehlers_ecema as ehlers_ecema_raw,
        ehlers_ecema_with_kernel,
    },
    ehlers_itrend::{EhlersITrendBatchBuilder, EhlersITrendInput, ehlers_itrend_with_kernel},
    ehlers_kama::{
        EhlersKamaBatchBuilder, EhlersKamaInput, ehlers_kama as ehlers_kama_raw,
        ehlers_kama_with_kernel,
    },
    ehlers_pma::{
        EhlersPmaBuilder, EhlersPmaInput, ehlers_pma as ehlers_pma_raw, ehlers_pma_with_kernel,
    },
    ...
    ehma::{EhmaBatchBuilder, EhmaInput, ehma as ehma_raw, ehma_with_kernel},
    ...
    ema::{EmaBatchBuilder, EmaInput, ema as ema_raw, ema_with_kernel},
    ema_deviation_corrected_t3::{
        EmaDeviationCorrectedT3Input, ema_deviation_corrected_t3 as ema_deviation_corrected_t3_raw,
        ema_deviation_corrected_t3_with_kernel,
    },
    epma::{EpmaBatchBuilder, EpmaInput, epma as epma_raw, epma_with_kernel},
    frama::{FramaBatchBuilder, FramaInput, frama_with_kernel},
    fwma::{FwmaBatchBuilder, FwmaInput, fwma as fwma_raw, fwma_with_kernel},
    gaussian::{
        GaussianBatchBuilder, GaussianInput, gaussian as gaussian_raw, gaussian_with_kernel,
    },
    highpass::{HighPassBatchBuilder, HighPassInput, highpass_with_kernel},
    highpass_2_pole::{HighPass2BatchBuilder, HighPass2Input, highpass_2_pole_with_kernel},
    hma::{HmaBatchBuilder, HmaInput, hma_with_kernel},
    hwma::{HwmaBatchBuilder, HwmaInput, hwma_with_kernel},
    jma::{JmaBatchBuilder, JmaInput, jma_with_kernel},
    jsa::{JsaBatchBuilder, JsaInput, jsa_with_kernel},
    kama::{KamaBatchBuilder, KamaInput, kama_with_kernel},
    linreg::{LinRegBatchBuilder, LinRegInput, linreg_with_kernel},
    maaq::{MaaqBatchBuilder, MaaqInput, maaq_with_kernel},
    mama::{MamaBatchBuilder, MamaInput, mama_with_kernel},
    mwdx::{MwdxBatchBuilder, MwdxInput, mwdx_with_kernel},
    nama::{NamaBatchBuilder, NamaInput, nama_with_kernel},
    nma::{NmaBatchBuilder, NmaInput, nma_with_kernel},
    pwma::{PwmaBatchBuilder, PwmaInput, pwma_with_kernel},
    reflex::{ReflexBatchBuilder, ReflexInput, reflex_with_kernel},
    sama::{SamaBatchBuilder, SamaInput, sama_with_kernel},
    sinwma::{SinWmaBatchBuilder, SinWmaInput, sinwma_with_kernel},
    sma::{SmaBatchBuilder, SmaInput, sma_with_kernel},
    smma::{SmmaBatchBuilder, SmmaInput, smma_with_kernel},
    sqwma::{SqwmaBatchBuilder, SqwmaInput, sqwma_with_kernel},
    srwma::{SrwmaBatchBuilder, SrwmaInput, srwma_with_kernel},
    supersmoother::{SuperSmootherBatchBuilder, SuperSmootherInput, supersmoother_with_kernel},
    supersmoother_3_pole::{
        SuperSmoother3PoleBatchBuilder, SuperSmoother3PoleInput, supersmoother_3_pole_with_kernel,
    },
    swma::{SwmaBatchBuilder, SwmaInput, swma_with_kernel},
    tema::{TemaBatchBuilder, TemaInput, tema_with_kernel},
    tilson::{TilsonBatchBuilder, TilsonInput, tilson_with_kernel},
    tradjema::{TradjemaInput, tradjema_with_kernel},
    trendflex::{TrendFlexBatchBuilder, TrendFlexInput, trendflex_with_kernel},
    trima::{TrimaBatchBuilder, TrimaInput, trima_with_kernel},
    uma::{UmaBatchBuilder, UmaInput, uma_with_kernel},
    volume_adjusted_ma::{
        VolumeAdjustedMa, VolumeAdjustedMa_with_kernel, VolumeAdjustedMaBatchBuilder,
        VolumeAdjustedMaInput,
    },
    vpwma::{VpwmaBatchBuilder, VpwmaInput, vpwma_with_kernel},
    vwma::{VwmaInput, VwmaParams, vwma_with_kernel},
    wilders::{WildersBatchBuilder, WildersInput, wilders_with_kernel},
    wma::{WmaBatchBuilder, WmaInput, wma_with_kernel},
    zlema::{ZlemaBatchBuilder, ZlemaInput, zlema_with_kernel},
};

use vector_ta::indicators::{
    bandpass::{
        BandPassBatchBuilder, BandPassInput, bandpass as bandpass_raw, bandpass_with_kernel,
    },
    cci_cycle::{CciCycleBatchBuilder, CciCycleInput, cci_cycle, cci_cycle_with_kernel},
    fvg_positioning_average::{
        FvgPositioningAverageInput, fvg_positioning_average as fvg_positioning_average_raw,
        fvg_positioning_average_with_kernel,
    },
    fvg_trailing_stop::{FvgTrailingStopInput, fvg_trailing_stop, fvg_trailing_stop_with_kernel},
    halftrend::{
        HalfTrendBatchBuilder, HalfTrendData, HalfTrendInput, halftrend, halftrend_with_kernel,
    },
    net_myrsi::{NetMyrsiBatchBuilder, NetMyrsiInput, net_myrsi, net_myrsi_with_kernel},
    reverse_rsi::{ReverseRsiBatchBuilder, ReverseRsiInput, reverse_rsi, reverse_rsi_with_kernel},
};

use vector_ta::indicators::moving_averages::volatility_adjusted_ma::{
    VamaBatchBuilder, VamaInput as VamaInputMv, vama, vama_with_kernel,
};

use vector_ta::indicators::correl_hl::correl_hl_with_kernel;
use vector_ta::indicators::{
    acosc::{AcoscInput, acosc as acosc_raw},
    ad::{AdInput, ad as ad_raw, ad_with_kernel},
    adosc::{AdoscInput, adosc as adosc_raw},
    adx::{AdxBatchBuilder, AdxInput, adx as adx_raw, adx_with_kernel},
    adxr::{AdxrInput, adxr as adxr_raw, adxr_with_kernel},
    alligator::{AlligatorInput, alligator as alligator_raw},
    alphatrend::{AlphaTrendInput, alphatrend as alphatrend_raw, alphatrend_with_kernel},
    ao::{AoInput, ao as ao_raw},
    apo::{ApoBatchBuilder, ApoInput, apo as apo_raw, apo_with_kernel},
    aroon::{AroonInput, aroon as aroon_raw},
    aroonosc::{AroonOscInput, aroon_osc as aroon_osc_raw},
    aso::{AsoBatchBuilder, AsoData, AsoInput, aso_with_kernel},
    atr::{AtrInput, atr as atr_raw},
    avsl::{AvslBatchBuilder, AvslInput, avsl_with_kernel},
    bollinger_bands::{
        BollingerBandsBatchBuilder, BollingerBandsInput, bollinger_bands as bollinger_bands_raw,
    ...
    bop::{BopInput, bop as bop_raw},
    cci::{CciInput, cci as cci_raw, cci_with_kernel},
    cfo::{CfoBatchBuilder, CfoInput, cfo as cfo_raw, cfo_with_kernel},
    cg::{CgInput, cg as cg_raw, cg_with_kernel},
    chande::{ChandeInput, chande as chande_raw},
    chandelier_exit::{CeBatchBuilder, ChandelierExitInput, chandelier_exit_with_kernel},
    chop::{ChopInput, chop as chop_raw},
    cksp::{CkspBatchBuilder, CkspInput, cksp as cksp_raw, cksp_with_kernel},
    cmo::{CmoBatchBuilder, CmoInput, cmo as cmo_raw, cmo_with_kernel},
    coppock::{CoppockInput, coppock as coppock_raw},
    cora_wave::{CoraWaveBatchBuilder, CoraWaveInput, cora_wave as cora_wave_raw},
    correl_hl::{CorrelHlBatchBuilder, CorrelHlData, CorrelHlInput, correl_hl as correl_hl_raw},
    correlation_cycle::{
    ...
    cvi::{CviBatchBuilder, CviInput, cvi as cvi_raw, cvi_with_kernel},
    damiani_volatmeter::{DamianiVolatmeterInput, damiani_volatmeter as damiani_volatmeter_raw},
    dec_osc::{DecOscInput, dec_osc as dec_osc_raw},
    decycler::{DecyclerInput, decycler as decycler_raw},
    demand_index::{DemandIndexInput, demand_index as demand_index_raw},
    deviation::{DeviationBatchBuilder, DeviationInput, deviation_with_kernel},
    devstop::{DevStopInput, devstop as devstop_raw},
    di::{DiInput, di as di_raw},
    dm::{DmBatchBuilder, DmInput, dm as dm_raw},
    donchian::{DonchianInput, donchian as donchian_raw, donchian_with_kernel},
    donchian_channel_width::{
        DonchianChannelWidthInput, donchian_channel_width as donchian_channel_width_raw,
        donchian_channel_width_with_kernel,
    },
    dpo::{DpoBatchBuilder, DpoInput, dpo as dpo_raw, dpo_with_kernel},
    dti::{DtiInput, dti as dti_raw, dti_with_kernel},
    dx::{DxBatchBuilder, DxInput, dx as dx_raw, dx_with_kernel},
    dynamic_momentum_index::{
        DynamicMomentumIndexInput, dynamic_momentum_index as dynamic_momentum_index_raw,
        dynamic_momentum_index_with_kernel,
    },
    efi::{EfiInput, efi as efi_raw, efi_with_kernel},
    ehlers_adaptive_cg::{
        EhlersAdaptiveCgInput, ehlers_adaptive_cg as ehlers_adaptive_cg_raw,
    ...
    ehlers_detrending_filter::{
        EhlersDetrendingFilterInput, ehlers_detrending_filter as ehlers_detrending_filter_raw,
        ehlers_detrending_filter_with_kernel,
    },
    ehlers_fm_demodulator::{
        EhlersFmDemodulatorInput, ehlers_fm_demodulator as ehlers_fm_demodulator_raw,
    ...
    emd::{EmdInput, emd as emd_raw, emd_with_kernel},
    emd_trend::{EmdTrendInput, emd_trend as emd_trend_raw, emd_trend_with_kernel},
    emv::{EmvBatchBuilder, EmvData, EmvInput, emv as emv_raw, emv_with_kernel},
    er::{ErBatchBuilder, ErInput, er as er_raw, er_with_kernel},
    eri::{EriBatchBuilder, EriData, EriInput, eri as eri_raw, eri_with_kernel},
    evasive_supertrend::{
        EvasiveSuperTrendInput, evasive_supertrend as evasive_supertrend_raw,
        evasive_supertrend_with_kernel,
    },
    ewma_volatility::{
        EwmaVolatilityInput, ewma_volatility as ewma_volatility_raw, ewma_volatility_with_kernel,
    },
    exponential_trend::{
        ExponentialTrendInput, exponential_trend as exponential_trend_raw,
        exponential_trend_with_kernel,
    },
    fibonacci_entry_bands::{
        FibonacciEntryBandsInput, fibonacci_entry_bands as fibonacci_entry_bands_raw,
        fibonacci_entry_bands_with_kernel,
    },
    fibonacci_trailing_stop::{
        FibonacciTrailingStopInput, fibonacci_trailing_stop as fibonacci_trailing_stop_raw,
        fibonacci_trailing_stop_with_kernel,
    },
    fisher::{FisherInput, fisher as fisher_raw, fisher_with_kernel},
    ...
    fosc::{FoscInput, fosc as fosc_raw, fosc_with_kernel},
    fractal_dimension_index::{
        FractalDimensionIndexInput, fractal_dimension_index as fractal_dimension_index_raw,
        fractal_dimension_index_with_kernel,
    },
    garman_klass_volatility::{
        GarmanKlassVolatilityInput, garman_klass_volatility as garman_klass_volatility_raw,
        garman_klass_volatility_with_kernel,
    },
    gatorosc::{GatorOscInput, gatorosc as gatorosc_raw},
    geometric_bias_oscillator::{
        GeometricBiasOscillatorInput, geometric_bias_oscillator as geometric_bias_oscillator_raw,
        geometric_bias_oscillator_with_kernel,
    },
    gmma_oscillator::{
        GmmaOscillatorInput, gmma_oscillator as gmma_oscillator_raw, gmma_oscillator_with_kernel,
    },
    ...
    ift_rsi::{IftRsiInput, ift_rsi as ift_rsi_raw, ift_rsi_with_kernel},
    kaufmanstop::{
        KaufmanstopBatchBuilder, KaufmanstopData, KaufmanstopInput, kaufmanstop as kaufmanstop_raw,
        kaufmanstop_with_kernel,
    },
    kdj::{KdjInput, kdj as kdj_raw, kdj_with_kernel},
    keltner::{KeltnerInput, keltner as keltner_raw, keltner_with_kernel},
    kst::{KstBatchBuilder, KstInput, kst as kst_raw},
    kurtosis::{
        KurtosisBatchBuilder, KurtosisInput, kurtosis as kurtosis_raw, kurtosis_with_kernel,
    },
    kvo::{KvoBatchBuilder, KvoInput, kvo as kvo_raw, kvo_with_kernel},
    linearreg_angle::{
        Linearreg_angleBatchBuilder, Linearreg_angleInput, linearreg_angle as linearreg_angle_raw,
        linearreg_angle_with_kernel,
    },
    linearreg_intercept::{
        LinearRegInterceptInput, linearreg_intercept as linearreg_intercept_raw,
    },
    linearreg_slope::{
        LinearRegSlopeBatchBuilder, LinearRegSlopeInput, linearreg_slope as linearreg_slope_raw,
        linearreg_slope_with_kernel,
    },
    lpc::{LpcInput, lpc as lpc_raw},
    lrsi::{LrsiInput, lrsi as lrsi_raw},
    mab::{MabInput, mab as mab_raw, mab_with_kernel},
    macd::{MacdInput, macd as macd_raw},
    macz::{MaczBatchBuilder, MaczInput, macz_with_kernel},
    marketefi::{MarketefiInput, marketefi as marketfi_raw, marketefi_with_kernel},
    mass::{MassInput, mass as mass_raw},
    mean_ad::{MeanAdInput, mean_ad as mean_ad_raw},
    medium_ad::{MediumAdInput, medium_ad as medium_ad_raw},
    medprice::{MedpriceInput, medprice as medprice_raw},
    mfi::{MfiBatchBuilder, MfiData, MfiInput, mfi as mfi_raw},
    midpoint::{MidpointInput, midpoint as midpoint_raw},
    midprice::{MidpriceBatchBuilder, MidpriceData, MidpriceInput, midprice as midprice_raw},
    minmax::{MinmaxInput, minmax as minmax_raw},
    mod_god_mode::{ModGodModeInput, mod_god_mode as mod_god_mode_raw, mod_god_mode_with_kernel},
    mom::{MomBatchBuilder, MomInput, mom as mom_raw, mom_with_kernel},
    msw::{MswInput, msw as msw_raw},
    nadaraya_watson_envelope::{
        NweInput, nadaraya_watson_envelope as nadaraya_watson_envelope_raw,
        nadaraya_watson_envelope_with_kernel,
    },
    natr::{NatrBatchBuilder, NatrInput, natr as natr_raw, natr_with_kernel},
    nvi::{NviInput, nvi as nvi_raw, nvi_with_kernel},
    obv::{ObvInput, obv as obv_raw},
    ott::{OttBatchBuilder, OttInput, ott as ott_raw, ott_with_kernel},
    otto::{OttoBatchBuilder, OttoInput, otto as otto_raw},
    pattern_recognition::{
        PatternRecognitionInput, pattern_recognition as pattern_recognition_raw,
    },
    percentile_nearest_rank::{
        PercentileNearestRankBatchBuilder, PercentileNearestRankInput,
        percentile_nearest_rank_with_kernel,
    },
    pfe::{PfeBatchBuilder, PfeInput, pfe as pfe_raw},
    pivot::{PivotBatchBuilder, PivotData, PivotInput, pivot as pivot_raw, pivot_with_kernel},
    pma::{PmaBatchBuilder, PmaInput, pma as pma_raw, pma_with_kernel},
    ppo::{PpoInput, ppo as ppo_raw, ppo_with_kernel},
    prb::{PrbBatchBuilder, PrbInput, prb as prb_raw},
    pvi::{PviBatchBuilder, PviInput, pvi as pvi_raw, pvi_with_kernel},
    qqe::{QqeInput, qqe as qqe_raw},
    qstick::{
        QstickBatchBuilder, QstickData, QstickInput, qstick as qstick_raw, qstick_with_kernel,
    },
    range_filter::{RangeFilterBatchBuilder, RangeFilterInput, range_filter_with_kernel},
    roc::{RocBatchBuilder, RocInput, roc as roc_raw, roc_with_kernel},
    rocp::{RocpInput, rocp as rocp_raw},
    rocr::{RocrInput, rocr as rocr_raw, rocr_with_kernel},
    rsi::{RsiBatchBuilder, RsiInput, rsi as rsi_raw, rsi_with_kernel},
    rsmk::{RsmkInput, rsmk as rsmk_raw},
    rsx::{RsxBatchBuilder, RsxInput, rsx as rsx_raw, rsx_with_kernel},
    rvi::{RviInput, rvi as rvi_raw, rvi_with_kernel},
    safezonestop::{
        SafeZoneStopBatchBuilder, SafeZoneStopData, SafeZoneStopInput,
        safezonestop as safezonestop_raw, safezonestop_with_kernel,
    },
    sar::{SarBatchBuilder, SarInput, sar as sar_raw, sar_with_kernel},
    squeeze_momentum::{
        SqueezeMomentumBatchBuilder, SqueezeMomentumInput, squeeze_momentum as squeeze_momentum_raw,
    },
    srsi::{SrsiBatchBuilder, SrsiInput, srsi as srsi_raw, srsi_with_kernel},
    stc::{StcInput, stc as stc_raw},
    stddev::{StdDevBatchBuilder, StdDevInput, stddev as stddev_raw},
    stoch::{StochBatchBuilder, StochInput, stoch as stoch_raw, stoch_with_kernel},
    stochf::{StochfInput, stochf as stochf_raw, stochf_with_kernel},
    supertrend::{
        SuperTrendBatchBuilder, SuperTrendInput, supertrend as supertrend_raw,
        supertrend_with_kernel,
    },
    trix::{TrixBatchBuilder, TrixInput, trix_with_kernel},
    tsf::{TsfInput, tsf as tsf_raw, tsf_with_kernel},
    tsi::{TsiInput, tsi as tsi_raw},
    ttm_squeeze::{TtmSqueezeInput, ttm_squeeze as ttm_squeeze_raw},
    ttm_trend::{TtmTrendInput, ttm_trend as ttm_trend_raw, ttm_trend_with_kernel},
    ui::{UiInput, ui as ui_raw, ui_with_kernel},
    ultosc::{
        UltOscBatchBuilder, UltOscBatchRange, UltOscData, UltOscInput, ultosc as ultosc_raw,
        ultosc_with_kernel,
    },
    var::{VarBatchBuilder, VarInput, var as var_raw, var_with_kernel},
    vi::{ViBatchBuilder, ViInput, vi as vi_raw},
    vidya::{VidyaBatchBuilder, VidyaInput, vidya_with_kernel},
    vlma::{VlmaBatchBuilder, VlmaInput, vlma_with_kernel},
    vosc::{VoscBatchBuilder, VoscInput, vosc as vosc_raw, vosc_with_kernel},
    voss::{VossInput, voss as voss_raw},
    vpci::{VpciBatchBuilder, VpciData, VpciInput, vpci as vpci_raw, vpci_with_kernel},
    vpt::{VptInput, vpt as vpt_raw, vpt_with_kernel},
    vwap::{VwapInput, vwap as vwap_raw},
    vwmacd::{VwmacdInput, vwmacd as vwmacd_raw},
    wad::{WadInput, wad as wad_raw},
    wavetrend::{
        WavetrendBatchBuilder, WavetrendInput, wavetrend as wavetrend_raw, wavetrend_with_kernel,
    },
    wclprice::{WclpriceInput, wclprice as wclprice_raw, wclprice_with_kernel},
    willr::{WillrBatchBuilder, WillrInput, willr as willr_raw},
    wto::{WtoBatchBuilder, WtoInput, wto_with_kernel},
    ...
    zscore::{ZscoreBatchBuilder, ZscoreInput, zscore as zscore_raw, zscore_with_kernel},
};

use vector_ta::indicators::dvdiqqe::{DvdiqqeInput, dvdiqqe_with_kernel};

use vector_ta::indicators::stc::stc_with_kernel;

use vector_ta::indicators::dm::dm_with_kernel;

use vector_ta::utilities::data_loader::{Candles, read_candles_from_vortex, source_type};

static BASE_CANDLES: Lazy<Arc<Candles>> = Lazy::new(|| {
    read_candles_from_vortex("src/data/2018-09-01-2024-Bitfinex_Spot-4h.vortex")
        .expect("load the immutable Vortex benchmark fixture")
});

fn benchmark_candles(len: usize) -> Candles {
    let source: &Candles = &BASE_CANDLES;
    assert!(source.close.len() > 1, "benchmark fixture is too short");
    let source_len = source.close.len();
    let timestamp_step = source.timestamp[1] - source.timestamp[0];
    let price_stride = source.close[source_len - 1] - source.open[0];
    ...
    for index in 0..len {
        ...
        let cycle = index / source_len;
        let price_offset = cycle as f64 * price_stride;
        timestamp.push(source.timestamp[0] + index as i64 * timestamp_step);
        open.push(source.open[source_index] + price_offset);
        high.push(source.high[source_index] + price_offset);
        low.push(source.low[source_index] + price_offset);
        close.push(source.close[source_index] + price_offset);
        volume.push(source.volume[source_index]);
    }
    Candles::new(timestamp, open, high, low, close, volume)
}

static CANDLES_10K: Lazy<Candles> = Lazy::new(|| benchmark_candles(10_000));
static CANDLES_100K: Lazy<Candles> = Lazy::new(|| benchmark_candles(100_000));
static CANDLES_1M: Lazy<Candles> = Lazy::new(|| benchmark_candles(1_000_000));
