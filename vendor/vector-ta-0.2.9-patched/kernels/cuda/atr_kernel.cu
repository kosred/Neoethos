...
static __forceinline__ __device__ double warp_reduce_sum_f64(double v) {
    unsigned mask = __activemask();
    #pragma unroll
    for (int offset = (warpSize >> 1); offset > 0; offset >>= 1) {
        v += __shfl_down_sync(mask, v, offset);
...
static __forceinline__ __device__ double block_reduce_sum_f64(double v) {
    ...
    if (wid == 0) {
        const int num_warps = (blockDim.x + warpSize - 1) >> 5;
        block_sum = (lane < num_warps) ? warp_sums[lane] : 0.0;
        block_sum = warp_reduce_sum_f64(block_sum);
    }
    return block_sum;
}
static __forceinline__ __device__ double tr_at_branchless_f64(
    const double hi, const double lo, const double pc, int t, int first_valid)
{
    const double seed = hi - lo;
    const double alt1 = fabs(hi - pc);
    const double alt2 = fabs(lo - pc);
    double m = fmax(seed, fmax(alt1, alt2));
    return (t == first_valid) ? seed : m;
}
static __forceinline__ __device__ void acc_add_f64(double &hi, double &lo, double x) {
    double s = hi + x;
    double bp = s - hi;
    double err = (hi - (s - bp)) + (x - bp);
    hi = s;
    lo += err;
}
static __forceinline__ __device__ void acc_add2_f64(double &hi, double &lo, double bhi, double blo) {
    acc_add_f64(hi, lo, bhi);
    acc_add_f64(hi, lo, blo);
}
static __forceinline__ __device__ void acc_sub2_f64(double ahi, double alo, double bhi, double blo,
                                                double &rhi, double &rlo) {
    rhi = 0.0; rlo = 0.0;
    acc_add_f64(rhi, rlo, ahi);
    acc_add_f64(rhi, rlo, alo);
    acc_add_f64(rhi, rlo, -bhi);
    acc_add_f64(rhi, rlo, -blo);
}
extern "C" __global__
void tr_from_hlc_f64(const double* __restrict__ high,
                     const double* __restrict__ low,
                     const double* __restrict__ close,
                     int series_len,
                     int first_valid,
                     double* __restrict__ tr_out)
{
    for (int t = blockIdx.x * blockDim.x + threadIdx.x;
         t < series_len;
         t += blockDim.x * gridDim.x)
    {
        double tri = 0.0;
        if (t >= first_valid) {
            const double hi = high[t];
            const double lo = low[t];
            const double pc = (t == first_valid) ? 0.0 : close[t - 1];
            tri = tr_at_branchless_f64(hi, lo, pc, t, first_valid);
        }
        tr_out[t] = tri;
    }
}
extern "C" __global__
void exclusive_prefix_float2_from_tr_f64(const double* __restrict__ tr,
                                     int series_len,
                                     double2* __restrict__ prefix2)
{
    if (blockIdx.x != 0) return;
    double hi = 0.0, lo = 0.0;
    if (threadIdx.x == 0) {
        prefix2[0] = make_double2(0.0, 0.0);
        for (int t = 0; t < series_len; ++t) {
            acc_add_f64(hi, lo, tr[t]);
            prefix2[t + 1] = make_double2(hi, lo);
        }
    }
}
extern "C" __global__
void atr_batch_unified_f64(const double* __restrict__ high,
                           const double* __restrict__ low,
                           const double* __restrict__ close,
                           const double* __restrict__ tr,
                           const double2* __restrict__ prefix2,
                           const int* __restrict__ periods,
                           ...
                           double* __restrict__ out)
{
    ...
    double seed_mean = 0.0;
    if (prefix2 != nullptr) {
        if (threadIdx.x == 0) {
            double2 a = prefix2[warm + 1];
            double2 b = prefix2[start];
            double shi, slo;
            acc_sub2_f64(a.x, a.y, b.x, b.y, shi, slo);
            seed_mean = (shi + slo) / (double)period;
        }
        __syncthreads();
    } else {
        double local = 0.0;
        ...
        for (int k = threadIdx.x; k < period; k += blockDim.x) {
            ...
            if (tr != nullptr) {
                ...
                const double hi = high[t];
                const double lo = low[t];
                const double pc = (t == start) ? 0.0 : close[t - 1];
                tri = tr_at_branchless_f64(hi, lo, pc, t, start);
            }
            local += tri;
        }
        const double sum = block_reduce_sum_f64(local);
        if (threadIdx.x == 0) seed_mean = sum / (double)period;
        __syncthreads();
    }
    ...
    if (threadIdx.x == 0) {
        ...
        for (int t = warm + 1; t < series_len; ++t) {
            ...
            if (tr != nullptr) {
                ...
                const double hi = high[t];
                const double lo = low[t];
                const double pc = close[t - 1];
                tri = tr_at_branchless_f64(hi, lo, pc, t, start);
            }
            y = __fma_rn(tri - y, alpha, y);
            out[base + t] = y;
        }
    }
}
extern "C" __global__
void atr_batch_f64(const double* __restrict__ high,
                   const double* __restrict__ low,
                   const double* __restrict__ close,
                   const int* __restrict__ periods,
                   const double* __restrict__ alphas,
                   ...
                   double* __restrict__ out)
{
    ...
    for (int k = threadIdx.x; k < period; k += blockDim.x) {
        ...
        const double lo = low[t];
        const double pc = (t == start) ? 0.0 : close[t - 1];
        local += tr_at_branchless_f64(hi, lo, pc, t, start);
    }
    const double sum = block_reduce_sum_f64(local);

    if (threadIdx.x == 0) {
        double y = sum / (double)period;
        out[base + warm] = y;
        for (int t = warm + 1; t < series_len; ++t) {
            const double hi = high[t];
            const double lo = low[t];
            const double pc = close[t - 1];
            const double tri = tr_at_branchless_f64(hi, lo, pc, t, start);
            y = __fma_rn(tri - y, alpha, y);
            out[base + t] = y;
        }
    }
...
extern "C" __global__
void atr_batch_from_tr_prefix_f64(const double* __restrict__ tr,
                                  const double* __restrict__ prefix_tr,
                                  const int* __restrict__ periods,
                                  const double* __restrict__ alphas,
                                  const int* __restrict__ warm_indices,
                                  ...
                                  double* __restrict__ out)
{
    ...
    if (prefix_tr != nullptr) {
        if (threadIdx.x == 0) {
            ...
            const double a = (double)prefix_tr[warm + 1];
            const double b = (double)prefix_tr[start];
            seed_mean = (a - b) / (double)period;
        }
        __syncthreads();
    } else {
        double local = 0.0;
...
extern "C" __global__
void atr_many_series_one_param_f64(const double* __restrict__ high_tm,
                                   ...
                                   double* __restrict__ out_tm)
{
    ...
    for (int s_base = warp_global * warpSize; s_base < num_series; s_base += warps_per_block * gridDim.x * warpSize) {
        ...
        for (int k = 0; k < period; ++k) {
            const int t = first_valid + k;
            const double hi = high_tm[t * stride + s];
            const double lo = low_tm[t * stride + s];
            const double pc = (t == first_valid) ? 0.0 : close_tm[(t - 1) * stride + s];
            sum += tr_at_branchless_f64(hi, lo, pc, t, first_valid);
        }
        ...
        for (int t = warm + 1; t < series_len; ++t) {
            const double hi = high_tm[t * stride + s];
            const double lo = low_tm[t * stride + s];
            const double pc = close_tm[(t - 1) * stride + s];
            const double tri = tr_at_branchless_f64(hi, lo, pc, t, first_valid);
            y = __fma_rn(tri - y, alpha, y);
            out_tm[t * stride + s] = y;
        }
    }
