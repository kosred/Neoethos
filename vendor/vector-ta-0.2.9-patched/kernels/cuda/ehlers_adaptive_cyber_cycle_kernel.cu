...
//
// CPU reference: src/indicators/ehlers_adaptive_cyber_cycle.rs:495
// (ehlers_adaptive_cyber_cycle_with_kernel). The column this emits is cycle,
// which is what output_id == "value" resolves to (dispatch/cpu_batch.rs:10142).
//
// SHAPE: one thread per combo, bars ascending. FORCED sequential -- five rings
// (source, smooth, cycle, delta-phase, adaptive) plus four carried scalars
// (prev_ip, prev_p, prev_q1, prev_i1), and the adaptive stage's OWN
// coefficient a1 = 2/(p+1) is derived from the carried period, so the filter
// changes shape bar by bar.
//
// THE VALID-BAR COUNTER IS THE INDEX, NOT i: the CPU SKIPS a non-finite source
// bar without advancing any ring, so bar counts finite bars only and the
// bar < 7 seed test is not a test on the raw index. Reproduced exactly.
//
// PERIOD-INVARIANT. compute_ehlers_adaptive_cyber_cycle_batch
// (cpu_batch.rs:10125-10126) reads source and alpha and NEVER period, so five
// swept periods give five identical CPU columns and this kernel emits five
// identical rows. The CPU default alpha 0.07 is pinned below.
//
// SOURCE IS hl2, NOT close (cpu_batch.rs:10125), so the lane row declares
// F64InputKind::Hl2Slice.
//
// FIRST VALID IS NOT READ: the CPU has no warmup index -- it walks from bar 0
// and skips non-finite bars -- so the lane row declares
// F64FirstValidRule::Ignored.
//
// f64 END TO END: double literals, double fmin/fmax/isfinite, no f32-suffixed
// math function, no fast-math intrinsic. median3 returns NaN when any argument
// is non-finite rather than letting a comparison chain drop it.
// ---------------------------------------------------------------------------

#define NEO_EACC_ALPHA 0.07

extern "C" __global__ void ehlers_adaptive_cyber_cycle_neo_batch_f64(
    const double* __restrict__ data,
    int n,
    ...
    const double alpha = NEO_EACC_ALPHA;
    if (!isfinite(alpha) || alpha < 0.0 || alpha > 1.0) {
        return;
    }
