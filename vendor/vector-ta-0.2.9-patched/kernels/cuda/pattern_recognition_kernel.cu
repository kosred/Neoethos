...
__device__ __forceinline__ double pr_open_f64(double body_low, double body_high, int8_t direction) {
    return (direction >= 0) ? body_low : body_high;
}
__device__ __forceinline__ double pr_close_f64(double body_low, double body_high, int8_t direction) {
    return (direction >= 0) ? body_high : body_low;
}
__device__ __forceinline__ double pr_high_f64(double body_high, double upper_shadow) {
    return body_high + upper_shadow;
}
__device__ __forceinline__ double pr_low_f64(double body_low, double lower_shadow) {
    return body_low - lower_shadow;
}
extern "C" __global__ void pattern_features_kernel_f64(
    const double* __restrict__ open,
    const double* __restrict__ high,
    const double* __restrict__ low,
    const double* __restrict__ close,
    int len,
    double* __restrict__ body,
    double* __restrict__ body_low,
    double* __restrict__ body_high,
    double* __restrict__ range,
    double* __restrict__ upper_shadow,
    double* __restrict__ lower_shadow,
    int8_t* __restrict__ direction,
    uint8_t* __restrict__ body_gap_up,
    uint8_t* __restrict__ body_gap_down)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double lo = fmin(o, c);
        const double hi = fmax(o, c);
        ...
        if (i == 0) {
            ...
            const double prev_lo = fmin(open[i - 1], close[i - 1]);
            const double prev_hi = fmax(open[i - 1], close[i - 1]);
            body_gap_up[i] = (lo > prev_hi) ? 1 : 0;
            body_gap_down[i] = (hi < prev_lo) ? 1 : 0;
        }
    }
}
extern "C" __global__ void pattern_doji_predicate_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ range,
    int len,
    double threshold,
    uint8_t* __restrict__ out_mask)
{
    ...
    for (int i = tid; i < len; i += stride) {
        const double b = body[i];
        const double r = range[i];
        const bool finite_vals = isfinite(b) && isfinite(r);
        const bool hit = finite_vals && r > 0.0 && b <= (threshold * r);
        out_mask[i] = hit ? 1 : 0;
    }
}
extern "C" __global__ void pattern_rolling_stats_10_f64_kernel(
    const double* __restrict__ body,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
    const int8_t* __restrict__ direction,
    int len,
    double* __restrict__ body_avg10,
    double* __restrict__ body_avg5,
    double* __restrict__ upper_avg10,
    double* __restrict__ lower_avg10,
    double* __restrict__ max_shadow_avg10,
    double* __restrict__ belt_shadow_avg10,
    double* __restrict__ closing_shadow_avg10)
{
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    ...
    for (int i = tid; i < len; i += stride) {
        if (i < 10) {
            ...
            upper_avg10[i] = 0.0;
            lower_avg10[i] = 0.0;
            max_shadow_avg10[i] = 0.0;
            belt_shadow_avg10[i] = 0.0;
            closing_shadow_avg10[i] = 0.0;
        } else {
            double sum_body = 0.0;
            double sum_upper = 0.0;
            double sum_lower = 0.0;
            double sum_max_shadow = 0.0;
            double sum_belt_shadow = 0.0;
            double sum_closing_shadow = 0.0;
            for (int j = i - 10; j < i; ++j) {
                const double upper = upper_shadow[j];
                const double lower = lower_shadow[j];
                sum_body += body[j];
                sum_upper += upper;
                sum_lower += lower;
                sum_max_shadow += fmax(upper, lower);
                if (direction[j] >= 0) {
                    sum_belt_shadow += lower;
                    sum_closing_shadow += upper;
                } else {
            ...
            body_avg10[i] = sum_body * 0.1;
            upper_avg10[i] = sum_upper * 0.1;
            lower_avg10[i] = sum_lower * 0.1;
            max_shadow_avg10[i] = sum_max_shadow * 0.1;
            belt_shadow_avg10[i] = sum_belt_shadow * 0.1;
            closing_shadow_avg10[i] = sum_closing_shadow * 0.1;
        }

        if (i < 5) {
            body_avg5[i] = 0.0;
        } else {
            double sum_body5 = 0.0;
            for (int j = i - 5; j < i; ++j) {
                sum_body5 += body[j];
            }
            body_avg5[i] = sum_body5 * 0.2;
        }
    }
}
extern "C" __global__ void pattern_rows_simple10_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_avg10,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
    const double* __restrict__ upper_avg10,
    const double* __restrict__ max_shadow_avg10,
    int len,
    uint8_t* __restrict__ matrix,
    int cols,
    int row_cdldoji,
    ...
    int row_cdlspinningtop)
{
    ...
    for (int i = tid; i < len; i += stride) {
        const bool warmup = i < 10;
        const double b = body[i];
        const double avg_body = body_avg10[i];
        const double upper = upper_shadow[i];
        const double lower = lower_shadow[i];
        const double avg_upper = upper_avg10[i];
        const double avg_shadow = max_shadow_avg10[i];
...
extern "C" __global__ void pattern_rows_two_bar_body10_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_avg10,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const int8_t* __restrict__ direction,
    const uint8_t* __restrict__ body_gap_up,
    const uint8_t* __restrict__ body_gap_down,
    int len,
    ...
    int row_cdlhomingpigeon)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        if (row_cdldojistar >= 0) {
            bool hit = false;
            if (!warmup) {
                const double avg_long = body_avg10[i - 1];
                const double avg_doji = body_avg10[i];
                const int8_t prev_dir = direction[i - 1];
                hit = body[i - 1] > avg_long
                    && body[i] <= avg_doji
                    && ((prev_dir > 0 && body_gap_up[i] != 0u) || (prev_dir < 0 && body_gap_down[i] != 0u));
        ...
        if (row_cdlharami >= 0 || row_cdlharamicross >= 0) {
            ...
            if (!warmup) {
                const double avg_long = body_avg10[i - 1];
                const double avg_short = body_avg10[i];
                if (body[i - 1] > avg_long && body[i] <= avg_short) {
                    const double hi0 = body_high[i - 1];
                    const double lo0 = body_low[i - 1];
                    const double hi1 = body_high[i];
                    const double lo1 = body_low[i];
                    hit = (hi1 <= hi0 && lo1 >= lo0);
                }
            }
            if (row_cdlharami >= 0) {
...
extern "C" __global__ void pattern_rows_single_bar_shadow_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ body_avg10,
    const double* __restrict__ body_avg5,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
    const double* __restrict__ upper_avg10,
    const double* __restrict__ lower_avg10,
    const uint8_t* __restrict__ body_gap_up,
    const uint8_t* __restrict__ body_gap_down,
    int len,
    uint8_t* __restrict__ matrix,
    ...
    int row_cdlrickshawman)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const bool warmup11 = i < 11;
        const double b = body[i];
        const double body_avg_i = body_avg10[i];
        const double upper_i = upper_shadow[i];
        const double lower_i = lower_shadow[i];
        const double upper_avg_i = upper_avg10[i];
        const double lower_avg_i = lower_avg10[i];

        if (row_cdlhammer >= 0) {
            bool hit = false;
            if (!warmup11) {
                const double prev_low = pr_low_f64(body_low[i - 1], lower_shadow[i - 1]);
                hit = b < body_avg_i
                    && lower_i > lower_avg_i
                    && upper_i < upper_avg_i
                    && body_low[i] <= prev_low + body_avg10[i - 1];
        ...
        if (row_cdlhangingman >= 0) {
            bool hit = false;
            if (!warmup11) {
                const double prev_high = pr_high_f64(body_high[i - 1], upper_shadow[i - 1]);
                hit = b < body_avg_i
                    && lower_i > lower_avg_i
                    && upper_i < upper_avg_i
                    && body_low[i] >= prev_high - body_avg10[i - 1];
        ...
        if (row_cdlrickshawman >= 0) {
            bool hit = false;
            if (!warmup10) {
                const double high_i = pr_high_f64(body_high[i], upper_i);
                const double low_i = pr_low_f64(body_low[i], lower_i);
                const double mid = low_i + (high_i - low_i) * 0.5;
                hit = b <= body_avg_i
                    && lower_i > upper_avg_i
                    && upper_i > upper_avg_i
                    && body_low[i] <= mid + body_avg5[i]
...
extern "C" __global__ void pattern_rows_directional_shadow_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_avg10,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
    const double* __restrict__ max_shadow_avg10,
    const double* __restrict__ belt_shadow_avg10,
    const double* __restrict__ closing_shadow_avg10,
    const int8_t* __restrict__ direction,
    const uint8_t* __restrict__ body_gap_up,
    const uint8_t* __restrict__ body_gap_down,
    int len,
    ...
    int row_cdlkickingbylength)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double upper_i = upper_shadow[i];
        const double lower_i = lower_shadow[i];
        const double max_shadow_i = fmax(upper_i, lower_i);

        if (row_cdlbelthold >= 0) {
            bool hit = false;
            if (!warmup10) {
                const double avg_shadow = belt_shadow_avg10[i];
                hit = b > body_avg10[i]
                    && ((dir_i >= 0 && lower_i < avg_shadow) || (dir_i < 0 && upper_i < avg_shadow));
            }
            matrix[row_cdlbelthold * cols + i] = hit ? 1u : 0u;
        ...
        if (row_cdlclosingmarubozu >= 0) {
            bool hit = false;
            if (!warmup10) {
                const double avg_shadow = closing_shadow_avg10[i];
                hit = b > body_avg10[i]
                    && ((dir_i >= 0 && upper_i < avg_shadow) || (dir_i < 0 && lower_i < avg_shadow));
            }
            matrix[row_cdlclosingmarubozu * cols + i] = hit ? 1u : 0u;
        ...
        if (row_cdlkicking >= 0 || row_cdlkickingbylength >= 0) {
            ...
            if (!warmup11) {
                ...
                hit = dir_prev == -dir_i
                    && body[i - 1] > body_avg10[i - 1]
                    && fmax(upper_shadow[i - 1], lower_shadow[i - 1]) < max_shadow_avg10[i - 1]
                    && b > body_avg10[i]
                    && max_shadow_i < max_shadow_avg10[i]
                    && gap_cond;
            }
...
extern "C" __global__ void pattern_rows_star3_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ body_avg10,
    ...
    int row_cdlmorningstar)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double close_i = pr_close_f64(body_low[i], body_high[i], direction[i]);
        const double close_prev2 = pr_close_f64(body_low[i - 2], body_high[i - 2], direction[i - 2]);
        const double avg_long = body_avg10[i - 2];
        const double avg_mid = body_avg10[i - 1];
        const double avg_short = body_avg10[i];
...
extern "C" __global__ void pattern_rolling_mean_f64_kernel(
    const double* __restrict__ input,
    int len,
    int period,
    double* __restrict__ out_avg)
{
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        out_avg[i] = sum / (double)period;
    }
}
extern "C" __global__ void pattern_rolling_max_shadow_mean_f64_kernel(
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
    int len,
    int period,
    double* __restrict__ out_avg)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        double sum = 0.0;
        for (int j = i - period; j < i; ++j) {
            sum += fmax(upper_shadow[j], lower_shadow[j]);
        }
        out_avg[i] = sum / (double)period;
    }
}
extern "C" __global__ void pattern_row_cdldoji_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_avg10,
    int len,
    uint8_t* __restrict__ matrix,
...
extern "C" __global__ void pattern_row_cdldragonflydoji_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_avg10,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
...
extern "C" __global__ void pattern_row_cdlgravestonedoji_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_avg10,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
...
extern "C" __global__ void pattern_row_cdllongleggeddoji_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_avg10,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
...
extern "C" __global__ void pattern_row_cdlmarubozu_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_avg10,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
...
extern "C" __global__ void pattern_row_cdldojistar_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_avg10,
    const int8_t* __restrict__ direction,
    const uint8_t* __restrict__ body_gap_up,
...
extern "C" __global__ void pattern_row_cdlengulfing_u8_kernel_f64(
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const int8_t* __restrict__ direction,
    int len,
...
extern "C" __global__ void pattern_row_cdlharami_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_avg10,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
...
extern "C" __global__ void pattern_row_cdlhighwave_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_avg10,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
...
extern "C" __global__ void pattern_row_cdlinvertedhammer_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_avg10,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ upper_avg10,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg_body = body_avg10[i - 1];
        const double avg_upper = upper_avg10[i - 1];
        const double avg_lower = lower_avg10[i - 1];
        const bool hit = body[i] < avg_body
            && upper_shadow[i] > avg_upper
            && lower_shadow[i] < avg_lower
            && body_gap_down[i] != 0u;
...
extern "C" __global__ void pattern_row_cdllongline_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_avg10,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
...
extern "C" __global__ void pattern_row_cdlhomingpigeon_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const int8_t* __restrict__ direction,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg_long = sum_long / (double)period_long;
        const double avg_short = sum_short / (double)period_short;
        const bool hit = direction[i - 1] < 0
            && direction[i] < 0
            && body[i - 1] > avg_long
            && body[i] <= avg_short
...
extern "C" __global__ void pattern_row_cdlshootingstar_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
    const uint8_t* __restrict__ body_gap_up,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg_body = sum_body / (double)period_body_short;
        const double avg_upper = sum_upper / (double)period_shadow_long;
        const double avg_lower = sum_lower / (double)period_shadow_very_short;
        const bool hit = body[i] < avg_body
            && upper_shadow[i] > avg_upper
            && lower_shadow[i] < avg_lower
            && body_gap_up[i] != 0u;
...
extern "C" __global__ void pattern_row_cdlshortline_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_avg10,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
...
extern "C" __global__ void pattern_row_cdlspinningtop_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_avg10,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
...
extern "C" __global__ void pattern_row_cdltakuri_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
    int len,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg_body = sum_body / (double)period_body_doji;
        const double avg_upper = sum_upper / (double)period_shadow_very_short;
        const double avg_lower = sum_lower / (double)period_shadow_very_long;
        const bool hit = body[i] <= avg_body && upper_shadow[i] < avg_upper && lower_shadow[i] > avg_lower;
        matrix[base + i] = hit ? 1u : 0u;
    }
}
extern "C" __global__ void pattern_row_cdlbelthold_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
    const int8_t* __restrict__ direction,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg_body = sum_body / (double)period_body_long;
        const double avg_shadow = sum_shadow / (double)period_shadow_very_short;
        const bool hit = body[i] > avg_body
            && ((direction[i] >= 0 && lower_shadow[i] < avg_shadow)
                || (direction[i] < 0 && upper_shadow[i] < avg_shadow));
        matrix[base + i] = hit ? 1u : 0u;
...
extern "C" __global__ void pattern_row_cdlclosingmarubozu_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
    const int8_t* __restrict__ direction,
...
extern "C" __global__ void pattern_row_cdlhammer_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg_body = sum_body / (double)period_body_short;
        const double avg_ls = sum_ls / (double)period_shadow_long;
        const double avg_us = sum_us / (double)period_shadow_very_short;
        const double avg_near = sum_near / (double)period_near;
        const double prev_low = pr_low_f64(body_low[i - 1], lower_shadow[i - 1]);
        const bool hit = body[i] < avg_body
            && lower_shadow[i] > avg_ls
            && upper_shadow[i] < avg_us
...
extern "C" __global__ void pattern_row_cdlhangingman_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
...
extern "C" __global__ void pattern_row_cdlrickshawman_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg_upper = sum_upper / (double)period_shadow_long;
        const double avg_near = sum_near / (double)period_near;
        const double high_i = pr_high_f64(body_high[i], upper_shadow[i]);
        const double low_i = pr_low_f64(body_low[i], lower_shadow[i]);
        const double mid = low_i + (high_i - low_i) * 0.5;
        const bool hit = body[i] <= avg_body
            && lower_shadow[i] > avg_upper
            && upper_shadow[i] > avg_upper
...
extern "C" __global__ void pattern_row_cdlmatchinglow_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const int8_t* __restrict__ direction,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        double sum_equal = 0.0;
        for (int j = i - period_equal - 1; j < i - 1; ++j) {
            sum_equal += body[j];
        }

        const double avg_equal = sum_equal / (double)period_equal;
        const double close_i = pr_close_f64(body_low[i], body_high[i], direction[i]);
        const double close_prev = pr_close_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const bool hit = direction[i - 1] < 0
            && direction[i] < 0
            && fabs(close_i - close_prev) <= avg_equal;
        matrix[base + i] = hit ? 1u : 0u;
    }
}
extern "C" __global__ void pattern_row_cdlinneck_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ lower_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg_equal = sum_equal / (double)period_equal;
        const double avg_long = sum_long / (double)period_body_long;
        const double open_i = pr_open_f64(body_low[i], body_high[i], direction[i]);
        const double close_i = pr_close_f64(body_low[i], body_high[i], direction[i]);
        const double close_prev = pr_close_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const double low_prev = pr_low_f64(body_low[i - 1], lower_shadow[i - 1]);
        const bool hit = direction[i - 1] < 0
            && body[i - 1] > avg_long
            && direction[i] >= 0
            && open_i < low_prev
...
extern "C" __global__ void pattern_row_cdlonneck_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ lower_shadow,
...
extern "C" __global__ void pattern_row_cdlpiercing_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ lower_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg_prev = sum_prev / (double)period_body_long;
        const double avg_curr = sum_curr / (double)period_body_long;
        const double open_i = pr_open_f64(body_low[i], body_high[i], direction[i]);
        const double close_i = pr_close_f64(body_low[i], body_high[i], direction[i]);
        const double open_prev = pr_open_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const double close_prev = pr_close_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const double low_prev = pr_low_f64(body_low[i - 1], lower_shadow[i - 1]);
        const bool hit = direction[i - 1] < 0
            && body[i - 1] > avg_prev
            ...
            && open_i < low_prev
            && close_i < open_prev
            && close_i > close_prev + body[i - 1] * 0.5;
        matrix[base + i] = hit ? 1u : 0u;
    }
}
extern "C" __global__ void pattern_row_cdlthrusting_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ lower_shadow,
...
extern "C" __global__ void pattern_row_cdleveningdojistar_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const int8_t* __restrict__ direction,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg_long = sum_long / (double)period_long;
        const double avg_doji = sum_doji / (double)period_doji;
        const double avg_short = sum_short / (double)period_short;
        const double close_i = pr_close_f64(body_low[i], body_high[i], direction[i]);
        const double close_prev2 = pr_close_f64(body_low[i - 2], body_high[i - 2], direction[i - 2]);
        const bool hit = body[i - 2] > avg_long
...
extern "C" __global__ void pattern_row_cdleveningstar_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const int8_t* __restrict__ direction,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        double sum_long = 0.0;
        double sum_short1 = 0.0;
        double sum_short0 = 0.0;
        for (int j = i - 2 - period_long; j < i - 2; ++j) {
            sum_long += body[j];
        }
        for (int j = i - 1 - period_short1; j < i - 1; ++j) {
        ...
        const double avg_long = sum_long / (double)period_long;
        const double avg_short1 = sum_short1 / (double)period_short1;
        const double avg_short0 = sum_short0 / (double)period_short0;
        const double close_i = pr_close_f64(body_low[i], body_high[i], direction[i]);
        const double close_prev2 = pr_close_f64(body_low[i - 2], body_high[i - 2], direction[i - 2]);
        const bool hit = body[i - 2] > avg_long
            && direction[i - 2] >= 0
...
extern "C" __global__ void pattern_row_cdlmorningdojistar_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const int8_t* __restrict__ direction,
...
extern "C" __global__ void pattern_row_cdlmorningstar_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const int8_t* __restrict__ direction,
...
extern "C" __global__ void pattern_row_cdlgapsidesidewhite_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const int8_t* __restrict__ direction,
...
extern "C" __global__ void pattern_row_cdlkicking_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
    const int8_t* __restrict__ direction,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        double sum_sh_prev = 0.0;
        double sum_sh_curr = 0.0;
        double sum_body_prev = 0.0;
        double sum_body_curr = 0.0;
        for (int j = i - 1 - period_shadow; j < i - 1; ++j) {
            sum_sh_prev += fmax(upper_shadow[j], lower_shadow[j]);
        }
        for (int j = i - period_shadow; j < i; ++j) {
            sum_sh_curr += fmax(upper_shadow[j], lower_shadow[j]);
        }
        for (int j = i - 1 - period_body; j < i - 1; ++j) {
            sum_body_prev += body[j];
        }
        ...
        const double avg_sh_prev = sum_sh_prev / (double)period_shadow;
        const double avg_sh_curr = sum_sh_curr / (double)period_shadow;
        const double avg_body_prev = sum_body_prev / (double)period_body;
        const double avg_body_curr = sum_body_curr / (double)period_body;
        const bool gap_cond = (direction[i - 1] < 0 && body_gap_up[i] != 0u)
            || (direction[i - 1] >= 0 && body_gap_down[i] != 0u);
        const bool hit = direction[i - 1] == -direction[i]
            && body[i - 1] > avg_body_prev
            && fmax(upper_shadow[i - 1], lower_shadow[i - 1]) < avg_sh_prev
            && body[i] > avg_body_curr
            && fmax(upper_shadow[i], lower_shadow[i]) < avg_sh_curr
            && gap_cond;
        matrix[base + i] = hit ? 1u : 0u;
    }
}
extern "C" __global__ void pattern_row_cdlkickingbylength_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
    const int8_t* __restrict__ direction,
...
extern "C" __global__ void pattern_row_cdlidentical3crows_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ lower_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg_sh2 = sum_sh2 / (double)period_shadow;
        const double avg_sh1 = sum_sh1 / (double)period_shadow;
        const double avg_sh0 = sum_sh0 / (double)period_shadow;
        const double avg_eq2 = sum_eq2 / (double)period_equal;
        const double avg_eq1 = sum_eq1 / (double)period_equal;
        const double close2 = pr_close_f64(body_low[i - 2], body_high[i - 2], direction[i - 2]);
        const double close1 = pr_close_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const double close0 = pr_close_f64(body_low[i], body_high[i], direction[i]);
        const double open1 = pr_open_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const double open0 = pr_open_f64(body_low[i], body_high[i], direction[i]);
        const bool hit = direction[i - 2] < 0
            && direction[i - 1] < 0
            && direction[i] < 0
            && lower_shadow[i - 2] < avg_sh2
...
extern "C" __global__ void pattern_row_cdlsticksandwich_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ lower_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double close2 = pr_close_f64(body_low[i - 2], body_high[i - 2], direction[i - 2]);
        const double close0 = pr_close_f64(body_low[i], body_high[i], direction[i]);
        const double low1 = pr_low_f64(body_low[i - 1], lower_shadow[i - 1]);
        const bool hit = direction[i - 2] < 0
            && direction[i - 1] >= 0
            && direction[i] < 0
            && low1 > close2
...
extern "C" __global__ void pattern_row_cdlseparatinglines_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        double sum_equal = 0.0;
        for (int j = i - period_shadow; j < i; ++j) {
            sum_shadow += fmax(upper_shadow[j], lower_shadow[j]);
        }
        for (int j = i - period_body_long; j < i; ++j) {
            sum_body += body[j];
        }
        ...
        const double avg_shadow = sum_shadow / (double)period_shadow;
        const double avg_body = sum_body / (double)period_body_long;
        const double avg_equal = sum_equal / (double)period_equal;
        const double open_i = pr_open_f64(body_low[i], body_high[i], direction[i]);
        const double open_prev = pr_open_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
...
extern "C" __global__ void pattern_row_cdlcounterattack_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const int8_t* __restrict__ direction,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        double sum_equal = 0.0;
        double sum_long_prev = 0.0;
        double sum_long_curr = 0.0;
        for (int j = i - period_equal - 1; j < i - 1; ++j) {
            sum_equal += body[j];
        }
        for (int j = i - period_body_long - 1; j < i - 1; ++j) {
        ...
        const double avg_equal = sum_equal / (double)period_equal;
        const double avg_long_prev = sum_long_prev / (double)period_body_long;
        const double avg_long_curr = sum_long_curr / (double)period_body_long;
        const double close_i = pr_close_f64(body_low[i], body_high[i], direction[i]);
        const double close_prev = pr_close_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const bool hit = direction[i] == -direction[i - 1]
            && body[i - 1] > avg_long_prev
...
extern "C" __global__ void pattern_row_cdldarkcloudcover_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double open_prev = pr_open_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const double close_prev = pr_close_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const double high_prev = pr_high_f64(body_high[i - 1], upper_shadow[i - 1]);
        const bool hit = direction[i - 1] >= 0
            && body[i - 1] > avg_long
            && direction[i] < 0
            && open_i > high_prev
...
extern "C" __global__ void pattern_row_cdlxsidegap3methods_u8_kernel_f64(
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const int8_t* __restrict__ direction,
    const uint8_t* __restrict__ body_gap_up,
...
extern "C" __global__ void pattern_row_cdlupsidegap2crows_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const int8_t* __restrict__ direction,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg_long = sum_long / (double)period_body_long;
        const double avg_short = sum_short / (double)period_body_short;
        const double open_i = pr_open_f64(body_low[i], body_high[i], direction[i]);
        const double close_i = pr_close_f64(body_low[i], body_high[i], direction[i]);
        const double open_prev = pr_open_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const double close_prev = pr_close_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
...
extern "C" __global__ void pattern_row_cdlunique3river_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ lower_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double close_prev = pr_close_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const double open_prev = pr_open_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const double open_prev2 = pr_open_f64(body_low[i - 2], body_high[i - 2], direction[i - 2]);
        const double low_prev = pr_low_f64(body_low[i - 1], lower_shadow[i - 1]);
        const double low_prev2 = pr_low_f64(body_low[i - 2], lower_shadow[i - 2]);
        const double open_i = pr_open_f64(body_low[i], body_high[i], direction[i]);
        const bool hit = body[i - 2] > avg_long
            && direction[i - 2] < 0
            && direction[i - 1] < 0
...
extern "C" __global__ void pattern_row_cdltasukigap_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const int8_t* __restrict__ direction,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const bool up_variant = body_gap_up[i - 1] != 0u
            ...
            && open_i > open_prev
            && close_i < open_prev
            && close_i > fmax(close_prev2, open_prev2)
            && fabs(body[i - 1] - body[i]) < avg_near;
        ...
        const bool down_variant = body_gap_down[i - 1] != 0u
            ...
            && open_i > close_prev
            && close_i > open_prev
            && close_i < fmin(close_prev2, open_prev2)
            && fabs(body[i - 1] - body[i]) < avg_near;

        matrix[base + i] = (up_variant || down_variant) ? 1u : 0u;
    }
}
extern "C" __global__ void pattern_row_cdlladderbottom_u8_kernel_f64(
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
    const int8_t* __restrict__ direction,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double close_i = pr_close_f64(body_low[i], body_high[i], direction[i]);
        const double high_prev = pr_high_f64(body_high[i - 1], upper_shadow[i - 1]);
        const double open4 = pr_open_f64(body_low[i - 4], body_high[i - 4], direction[i - 4]);
        const double open3 = pr_open_f64(body_low[i - 3], body_high[i - 3], direction[i - 3]);
        const double open2 = pr_open_f64(body_low[i - 2], body_high[i - 2], direction[i - 2]);
        const double close4 = pr_close_f64(body_low[i - 4], body_high[i - 4], direction[i - 4]);
        const double close3 = pr_close_f64(body_low[i - 3], body_high[i - 3], direction[i - 3]);
        const double close2 = pr_close_f64(body_low[i - 2], body_high[i - 2], direction[i - 2]);
        const bool hit = direction[i - 4] < 0
            && direction[i - 3] < 0
            && direction[i - 2] < 0
...
extern "C" __global__ void pattern_row_cdlstalledpattern_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        double sum_long2 = 0.0;
        double sum_long1 = 0.0;
        double sum_short0 = 0.0;
        double sum_shadow1 = 0.0;
        double sum_near2 = 0.0;
        double sum_near1 = 0.0;
        for (int j = i - period_body_long; j < i; ++j) {
            sum_long2 += body[j - 2];
            sum_long1 += body[j - 1];
        }
        ...
        const double avg_long2 = sum_long2 / (double)period_body_long;
        const double avg_long1 = sum_long1 / (double)period_body_long;
        const double avg_short0 = sum_short0 / (double)period_body_short;
        const double avg_shadow1 = sum_shadow1 / (double)period_shadow_very_short;
        const double avg_near2 = sum_near2 / (double)period_near;
        const double avg_near1 = sum_near1 / (double)period_near;
        const double close_i = pr_close_f64(body_low[i], body_high[i], direction[i]);
        const double close_1 = pr_close_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const double close_2 = pr_close_f64(body_low[i - 2], body_high[i - 2], direction[i - 2]);
        const double open_1 = pr_open_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const double open_2 = pr_open_f64(body_low[i - 2], body_high[i - 2], direction[i - 2]);
        const double open_i = pr_open_f64(body_low[i], body_high[i], direction[i]);
        const bool hit = direction[i - 2] >= 0
            && direction[i - 1] >= 0
            && direction[i] >= 0
...
extern "C" __global__ void pattern_row_cdlhikkake_u8_kernel_f64(
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
    ...
    int row)
{
    ...
    for (int i = 5; i < len; ++i) {
        ...
        if (pattern_idx != 0 && i <= (pattern_idx + 3)) {
            const double close_i = pr_close_f64(body_low[i], body_high[i], direction[i]);
            const double high_p1 = pr_high_f64(body_high[pattern_idx - 1], upper_shadow[pattern_idx - 1]);
            const double low_p1 = pr_low_f64(body_low[pattern_idx - 1], lower_shadow[pattern_idx - 1]);
            if ((pattern_result > 0 && close_i > high_p1) || (pattern_result < 0 && close_i < low_p1)) {
                matrix[base + i] = 1;
                pattern_idx = 0;
            }
        ...
        const double high_1 = pr_high_f64(body_high[i - 1], upper_shadow[i - 1]);
        const double low_1 = pr_low_f64(body_low[i - 1], lower_shadow[i - 1]);
        const double high_2 = pr_high_f64(body_high[i - 2], upper_shadow[i - 2]);
        const double low_2 = pr_low_f64(body_low[i - 2], lower_shadow[i - 2]);
...
extern "C" __global__ void pattern_row_cdlhikkakemod_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
    ...
    int row)
{
    ...
    for (int i = lookback; i < len; ++i) {
        ...
        const double near_avg = sum_near / (double)period_near;
        const double high_3 = pr_high_f64(body_high[i - 3], upper_shadow[i - 3]);
        const double low_3 = pr_low_f64(body_low[i - 3], lower_shadow[i - 3]);
        const double high_2 = pr_high_f64(body_high[i - 2], upper_shadow[i - 2]);
        const double low_2 = pr_low_f64(body_low[i - 2], lower_shadow[i - 2]);
        const double high_1 = pr_high_f64(body_high[i - 1], upper_shadow[i - 1]);
        const double low_1 = pr_low_f64(body_low[i - 1], lower_shadow[i - 1]);
...
extern "C" __global__ void pattern_row_cdl2crows_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const int8_t* __restrict__ direction,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double first_open = pr_open_f64(body_low[i - 2], body_high[i - 2], direction[i - 2]);
        const double first_close = pr_close_f64(body_low[i - 2], body_high[i - 2], direction[i - 2]);
        const double second_open = pr_open_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const double second_close = pr_close_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const double third_open = pr_open_f64(body_low[i], body_high[i], direction[i]);
        const double third_close = pr_close_f64(body_low[i], body_high[i], direction[i]);
...
extern "C" __global__ void pattern_row_cdl3blackcrows_u8_kernel_f64(
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg2 = sum2 / (double)period_shadow_very_short;
        const double avg1 = sum1 / (double)period_shadow_very_short;
        const double avg0 = sum0 / (double)period_shadow_very_short;
        ...
        const double open0 = pr_open_f64(body_low[i], body_high[i], direction[i]);
        const double close0 = pr_close_f64(body_low[i], body_high[i], direction[i]);
        const double high3 = pr_high_f64(body_high[i - 3], upper_shadow[i - 3]);
...
extern "C" __global__ void pattern_row_cdl3inside_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const int8_t* __restrict__ direction,
...
extern "C" __global__ void pattern_row_cdl3linestrike_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const int8_t* __restrict__ direction,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg3 = sum3 / (double)period_near;
        const double avg2 = sum2 / (double)period_near;
...
extern "C" __global__ void pattern_row_cdl3outside_u8_kernel_f64(
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const int8_t* __restrict__ direction,
    int len,
...
extern "C" __global__ void pattern_row_cdl3starsinsouth_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        double sum_body_long = 0.0;
        double sum_shadow_long = 0.0;
        double sum_shadow_short_1 = 0.0;
        double sum_shadow_short_0 = 0.0;
        double sum_body_short = 0.0;
        ...
        const double avg_body_long = sum_body_long / (double)period_body_long;
        const double avg_shadow_long = sum_shadow_long / (double)period_shadow_long;
        const double avg_shadow_short_1 = sum_shadow_short_1 / (double)period_shadow_very_short;
        const double avg_shadow_short_0 = sum_shadow_short_0 / (double)period_shadow_very_short;
        const double avg_body_short = sum_body_short / (double)period_body_short;

        const double open1 = pr_open_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const double close2 = pr_close_f64(body_low[i - 2], body_high[i - 2], direction[i - 2]);
        const double low2 = pr_low_f64(body_low[i - 2], lower_shadow[i - 2]);
        const double low1 = pr_low_f64(body_low[i - 1], lower_shadow[i - 1]);
        const double low0 = pr_low_f64(body_low[i], lower_shadow[i]);
        const double high2 = pr_high_f64(body_high[i - 2], upper_shadow[i - 2]);
        const double high1 = pr_high_f64(body_high[i - 1], upper_shadow[i - 1]);
        const double high0 = pr_high_f64(body_high[i], upper_shadow[i]);
...
extern "C" __global__ void pattern_row_cdl3whitesoldiers_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg_sv2 = sum_sv2 / (double)period_shadow_very_short;
        const double avg_sv1 = sum_sv1 / (double)period_shadow_very_short;
        const double avg_sv0 = sum_sv0 / (double)period_shadow_very_short;
        const double avg_near2 = sum_near2 / (double)period_near;
        const double avg_near1 = sum_near1 / (double)period_near;
        const double avg_far2 = sum_far2 / (double)period_far;
        const double avg_far1 = sum_far1 / (double)period_far;
        const double avg_body_short = sum_body_short / (double)period_body_short;
...
extern "C" __global__ void pattern_row_cdlabandonedbaby_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg_long = sum_long / (double)period_body_long;
        const double avg_doji = sum_doji / (double)period_body_doji;
        const double avg_short = sum_short / (double)period_body_short;
...
extern "C" __global__ void pattern_row_cdladvanceblock_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        double sum_shadow_short_2 = 0.0;
        double sum_shadow_short_1 = 0.0;
        double sum_shadow_short_0 = 0.0;
        double sum_shadow_long_0 = 0.0;
        double sum_near_2 = 0.0;
        double sum_near_1 = 0.0;
        double sum_far_2 = 0.0;
        double sum_far_1 = 0.0;
        double sum_body_long = 0.0;
        ...
        const double avg_shadow_short_2 = sum_shadow_short_2 / (double)period_shadow_short;
        const double avg_shadow_short_1 = sum_shadow_short_1 / (double)period_shadow_short;
        const double avg_shadow_short_0 = sum_shadow_short_0 / (double)period_shadow_short;
        const double avg_shadow_long_0 = sum_shadow_long_0 / (double)period_shadow_long;
        const double avg_near_2 = sum_near_2 / (double)period_near;
        const double avg_near_1 = sum_near_1 / (double)period_near;
        const double avg_far_2 = sum_far_2 / (double)period_far;
        const double avg_far_1 = sum_far_1 / (double)period_far;
        const double avg_body_long = sum_body_long / (double)period_body_long;
...
extern "C" __global__ void pattern_row_cdlbreakaway_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double high2 = pr_high_f64(body_high[i - 2], upper_shadow[i - 2]);
        const double high1 = pr_high_f64(body_high[i - 1], upper_shadow[i - 1]);
        const double low3 = pr_low_f64(body_low[i - 3], lower_shadow[i - 3]);
        const double low2 = pr_low_f64(body_low[i - 2], lower_shadow[i - 2]);
        const double low1 = pr_low_f64(body_low[i - 1], lower_shadow[i - 1]);
...
extern "C" __global__ void pattern_row_cdlconcealbabyswall_u8_kernel_f64(
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
    const double* __restrict__ lower_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        double sum1 = 0.0;
        for (int j = i - 3 - period_shadow_very_short; j < i - 3; ++j) {
            sum3 += fmax(upper_shadow[j], lower_shadow[j]);
        }
        for (int j = i - 2 - period_shadow_very_short; j < i - 2; ++j) {
            sum2 += fmax(upper_shadow[j], lower_shadow[j]);
        }
        for (int j = i - 1 - period_shadow_very_short; j < i - 1; ++j) {
            sum1 += fmax(upper_shadow[j], lower_shadow[j]);
        }
        const double avg3 = sum3 / (double)period_shadow_very_short;
        const double avg2 = sum2 / (double)period_shadow_very_short;
        const double avg1 = sum1 / (double)period_shadow_very_short;
...
extern "C" __global__ void pattern_row_cdlmathold_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg4 = sum4 / (double)period_body_long;
        const double avg3 = sum3 / (double)period_body_short;
        const double avg2 = sum2 / (double)period_body_short;
        const double avg1 = sum1 / (double)period_body_short;
        ...
        const bool hit = body[i - 4] > avg4
            ...
            && body_high[i - 1] < body_high[i - 2]
            && open0 > close1
            && close0 > fmax(high3, fmax(high2, high1));
        matrix[base + i] = hit ? 1u : 0u;
    }
}
extern "C" __global__ void pattern_row_cdlrisefall3methods_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const double* __restrict__ upper_shadow,
    ...
    int row)
{
    ...
    for (int i = tid; i < len; i += stride) {
        ...
        const double avg2 = sum2 / (double)period_body_short;
        const double avg1 = sum1 / (double)period_body_short;
        const double avg0 = sum0 / (double)period_body_long;

        const int8_t c4 = direction[i - 4];
        const double s = (double)c4;
        const double close4 = pr_close_f64(body_low[i - 4], body_high[i - 4], direction[i - 4]);
        const double close3 = pr_close_f64(body_low[i - 3], body_high[i - 3], direction[i - 3]);
        const double close2 = pr_close_f64(body_low[i - 2], body_high[i - 2], direction[i - 2]);
        const double close1 = pr_close_f64(body_low[i - 1], body_high[i - 1], direction[i - 1]);
        const double open0 = pr_open_f64(body_low[i], body_high[i], direction[i]);
        const double close0 = pr_close_f64(body_low[i], body_high[i], direction[i]);
        const double high4 = pr_high_f64(body_high[i - 4], upper_shadow[i - 4]);
        const double low4 = pr_low_f64(body_low[i - 4], lower_shadow[i - 4]);
...
extern "C" __global__ void pattern_row_cdltristar_u8_kernel_f64(
    const double* __restrict__ body,
    const double* __restrict__ body_low,
    const double* __restrict__ body_high,
    const uint8_t* __restrict__ body_gap_up,
...
extern "C" __global__ void pattern_u8_to_f64_kernel(
    const uint8_t* __restrict__ matrix_u8,
    double* __restrict__ matrix_f32,
    int total)
{
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    ...
    for (int i = tid; i < total; i += stride) {
        matrix_f32[i] = matrix_u8[i] == 0u ? 0.0 : 1.0;
    }
}
