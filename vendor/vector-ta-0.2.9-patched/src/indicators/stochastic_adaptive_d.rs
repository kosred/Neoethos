...
pub enum StochasticAdaptiveDError {
    ...
    AllValuesNaN,
    #[error(
        "stochastic_adaptive_d: Invalid stochastic length: k_length = {k_length}, data length = {data_len}"
    )]
    InvalidKLength { k_length: usize, data_len: usize },
    #[error(
        "stochastic_adaptive_d: Invalid d_smoothing: d_smoothing = {d_smoothing}, data length = {data_len}"
    )]
    InvalidDSmoothing { d_smoothing: usize, data_len: usize },
    #[error(
        "stochastic_adaptive_d: Invalid pre_smooth: pre_smooth = {pre_smooth}, data length = {data_len}"
    )]
